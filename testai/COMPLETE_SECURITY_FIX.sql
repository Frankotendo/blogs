...
-- 7. ADD PASSWORD FIELD TO USERS TABLE
ALTER TABLE public.unihub_users 
ADD COLUMN IF NOT EXISTS password text;

-- 8. ADD PASSWORD FIELD TO DRIVERS TABLE  
ALTER TABLE public.unihub_drivers 
ADD COLUMN IF NOT EXISTS password text;

-- 9. ADD EMAIL FIELD TO DRIVERS TABLE FOR ADMIN LOGIN
ALTER TABLE public.unihub_drivers 
...
BEGIN
  ...
  IF stored_hash = answer_hash THEN
    RETURN NEXT SELECT true, 'Security answer verified'::text;
  ELSE
    RETURN NEXT SELECT false, 'Incorrect security answer'::text;
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
