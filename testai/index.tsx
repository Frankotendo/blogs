...
const DriverPortal = ({
  ...
  const [activeTab, setActiveTab] = useState<
    "market" | "active" | "wallet" | "broadcast"
  >("market");
  ...
  useEffect(() => {
    if (activeDriver && window.io) {
      const newSocket = window.io('https://kzjgihwxiaeqzopeuzhm.supabase.co/realtime', {
        auth: {
          apikey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imt6amdpaHd4aWFlcXpvcGV1emhtIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njk2OTU4MDMsImV4cCI6MjA4NTI3MTgwM30.G_6hWSgPstbOi9GgnGprZW9IQVFZSGPQnyC80RROmuw'
        }
      });
      ...
      newSocket.on('passengerLocationUpdate', (data: any) => {
        setPassengerLocations(prev => ({
          ...prev,
          [data.passengerId]: { lat: data.latitude, lng: data.longitude }
        }));
      });

      // Listen for ride assignments
      newSocket.on('rideAssigned', (data: any) => {
      ...
      {activeTab === "active" && (
        <div className="space-y-4">
          ...
          {myActiveRides.map((node: any) => {
            // Get real-time passenger locations for this ride
            const passengerGPSLocations = node.passengers?.map((p: any) => passengerLocations[p.id]) || [];
            const firstPassengerLocation = passengerGPSLocations[0];
              ...
              <div className="relative z-10">
                ...
                <div className="bg-black/30 p-6 rounded-2xl border border-white/10">
                  ...
                  <div className="space-y-3">
                    ...
                    <button
                      onClick={() => {
                        // Use real-time GPS location if available, otherwise fallback to address
                    ...
                    <div className="bg-black/50 rounded-lg p-4 border border-white/10">
                      <iframe
                        src={firstPassengerLocation 
                          ? `https://www.google.com/maps/dir/?api=1&origin=${firstPassengerLocation.lat},${firstPassengerLocation.lng}&destination=${encodeURIComponent(node.destination)}`
                          : `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(node.origin)}&destination=${encodeURIComponent(node.destination)}`
                        }
                        width="100%"
                        height="300"
                        style={{ border: 0 }}
  ...
  const handleGlobalUserAuth = async (
    ...
    try {
      if (mode === "login") {
        ...
        // Check PIN
        if (user.pin !== pin) {
          alert("Login failed. Invalid PIN.");
          setIsSyncing(false);
          return;
        }
...
// Successful login
  ...
  const handleCancelRide = async (rideId: string) => {
    ...
    try {
      await supabase.from("unihub_nodes").delete().eq("id", rideId);
  ...
  const handleSettleRide = async (rideId: string) => {
    try {
      await supabase.from("unihub_nodes").update({ status: "completed" }).eq("id", rideId);
