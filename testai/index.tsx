...
import { formatCoordinatesForDB, getValidGhanaCoordinate } from "./coordinate_utils";
import {
  GoogleGenAI,
  Type,
  Chat,
...
const GEMINI_KEY = typeof import.meta !== "undefined" ? (import.meta as any).env?.VITE_GEMINI_KEY : undefined;
      ...
      <div className="bg-white w-full max-w-sm rounded-[2rem] overflow-hidden relative shadow-2xl">
        ...
        <div className="p-6 space-y-4 text-center">
          ...
          <div className="grid grid-cols-2 gap-3 text-[9px] font-bold uppercase text-slate-500">
            <div className="flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-200">
              <i className="fas fa-camera-rotate text-emerald-600" />
              <span className="text-emerald-700">Camera switch enabled</span>
            </div>
...
const GlobalVoiceOrb = ({
  ...
  const toggleSession = async () => {
    ...
    if (!ai) {
      alert("Voice assistant is unavailable.");
      setIsActive(false);
      setState("idle");
      return;
    }
...
const DriverPortal = ({
  ...
  // Subscribe to passenger location updates via Supabase Realtime
  useEffect(() => {
    if (!activeDriver) return;
    const channel = supabase
      .channel('realtime:passenger_locations')
      .on(
        'postgres_changes',
        ...
        (payload: any) => {
          ...
          setPassengerLocations(prev => ({
            ...prev,
            [row.passenger_id]: { lat: Number(row.latitude), lng: Number(row.longitude) }
          }));
        }
      )
      .subscribe();
                    ...
                    <div className="bg-black/50 rounded-lg p-4 border border-white/10">
                      <iframe
                        src={firstPassengerLocation 
                          ...
                          : `https://www.google.com/maps?q=${encodeURIComponent(node.origin)}+to+${encodeURIComponent(node.destination)}`
                        }
                        width="100%"
                        height="300"
                        style={{ border: 0 }}
...
// Successful login
  ...
  const handleDriverAuth = async (driverId: string, pin: string) => {
    ...
    try {
      ...
      // Capture and save driver location
      if ('geolocation' in navigator) {
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            try {
              // Insert/update driver location in database with validation
              const validCoords = formatCoordinatesForDB(latitude, longitude);
              ...
              if (locationError) {
                console.error('Error saving driver location:', locationError);
              } else {
          ...
          (error) => {
            ...
            alert(`Welcome back, ${driver.name}! Location access denied - you may not appear on the map.`);
          },
          {
        ...
        alert(`Welcome back, ${driver.name}! Geolocation not supported - you may not appear on the map.`);
      }
  ...
  const handleDriverLogout = async () => {
    // Remove driver from location tracking when logging out
    if (activeDriverId) {
      try {
        await supabase
          .from('driver_locations')
        ...
        console.log('Driver location removed on logout');
      } catch (err) {
        console.error('Failed to remove driver location on logout:', err);
      }
    }
  ...
  // Continuous location tracking for logged-in drivers
  useEffect(() => {
    if (!activeDriverId || !('geolocation' in navigator)) return;
    ...
    const startLocationTracking = () => {
      watchId = navigator.geolocation.watchPosition(
        async (position) => {
          ...
          try {
            // Update driver location in database with coordinate validation
            const validCoords = formatCoordinatesForDB(latitude, longitude);
            const { error: locationError } = await supabase
              .from('driver_locations')
              .upsert({
  ...
  // Continuous location tracking for logged-in passengers with active rides
  const hasActiveRide = useMemo(() => {
    if (!currentUser) return false;
    return nodes.some(
      (n) => n.passengers?.some((p: any) => p.phone === currentUser.phone) && n.status !== "completed",
    );
  }, [nodes, currentUser]);

  useEffect(() => {
    if (!currentUser || !hasActiveRide || !("geolocation" in navigator)) return;
    let watchId: number = 0;
    watchId = navigator.geolocation.watchPosition(
      async (position) => {
        const { latitude, longitude, accuracy, heading, speed } = position.coords;
        try {
          // Validate coordinates for passenger location
          const validCoords = formatCoordinatesForDB(latitude, longitude);
          ...
          console.warn("passenger_locations upsert failed", e);
        }
      },
      (error) => {
        console.warn("Passenger geolocation error", error);
      },
