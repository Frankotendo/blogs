...
const App: React.FC = () => {
  ...
  const leaveNode = async (nodeId: string, phone: string) => {
    ...
    // LOGIC: Block passengers from leaving a dispatched ride to protect the driver's commitment
    if (node.status === 'dispatched') {
        alert("🚫 Cannot leave a dispatched ride.\n\nThe driver is already en route. Please ask the driver to verify you or contact the Admin to cancel this trip if necessary.");
        return;
    }
  ...
  const reportPassengerNoShow = async (nodeId: string, passengerPhone: string, passengerName: string) => {
     ...
     // Anti-Cheat: Ensure we are in a dispatched state and the user is the assigned driver
     if (node.assignedDriverId !== activeDriverId || node.status !== 'dispatched') {
         alert("Can only report no-show on your active dispatched trips.");
         return;
     }
     ...
     const refundReq: RefundRequest = {
         ...
         amount: rate,
         reason: `No-Show: ${passengerName} (${passengerPhone})`,
         status: 'pending',
         timestamp: new Date().toLocaleString()
     };

     try {
         // Execute Removal AND Create Refund Request
         await Promise.all([
         ...
         // If no passengers left, we might want to auto-cancel or ask driver
         if(newPassengers.length === 0) {
             if(confirm("Trip has 0 passengers left. Cancel trip and go Online?")) {
                 await cancelRide(nodeId);
             }
         }
     } catch(e: any) {
         console.error("No Show Error", e);
         alert("Error processing no-show: " + e.message);
     }
  };
  ...
  const acceptRide = async (nodeId: string, driverId: string, customFare?: number) => {
    ...
    // Logic: Driver cannot take a new ride if they are already assigned to a dispatched/active one
    ...
    // STRICT COMMISSION CALCULATION LOGIC
    // Uses specific shuttle rate if driver is a Shuttle, otherwise standard seat rate
    // FORCE NUMBER CONVERSION to prevent string concatenation bugs
    const commissionRate = Number((latestDriver.vehicleType === 'Shuttle' && settings.shuttleCommission) 
                           ? settings.shuttleCommission 
                           : settings.commissionPerSeat);
                           
    const totalCommission = commissionRate * node.passengers.length;

    // CONFIRMATION DIALOG to prevent "Wrong Deduction" Confusion
  ...
  const verifyRide = async (nodeId: string, code: string) => {
    ...
    if (isMasterCode || passengerMatch) {
      try {
        ...
        // Auto-set driver to online if they were the assigned driver
        if(node.assignedDriverId) {
             await supabase.from('unihub_drivers').update({ status: 'online' }).eq('id', node.assignedDriverId);
        }
        ...
        const successMsg = passengerMatch 
            ...
            : `Ride verified and completed! Driver set to Online.`;
  ...
  const cancelRide = async (nodeId: string) => {
    ...
    try {
      if (node.assignedDriverId) {
        ...
        if (node.status === 'dispatched') {
             // LOGIC: Full refund only happens if Admin cancels or system logic allows (Anti-Cheat blocked above for drivers)
             let refundAmount = 0;
             ...
             if (isShuttle && isBroadcast) {
                 ...
                 refundAmount = rate * node.passengers.length;
             }

             await Promise.all([
                 supabase.from('unihub_drivers').update({ 
                     walletBalance: latestDriver.walletBalance + refundAmount,
                     status: 'online' // Auto-Free the driver
        ...
        if (isBroadcast || (node.status === 'forming' && node.passengers.length === 0)) {
            // Logic for pre-dispatch cancellation (Refund valid here)
            if (node.status !== 'dispatched') {
                if (isShuttle && isBroadcast) {
                     const cap = node.capacityNeeded;
                    ...
                    // Just free the driver status even if no refund needed
                    await supabase.from('unihub_drivers').update({ status: 'online' }).eq('id', latestDriver.id);
                }
            }
        ...
        // If Admin cancels, free the driver completely
        ...
        // Explicitly set driver online when unassigning
        if (latestDriver) {
            await supabase.from('unihub_drivers').update({ status: 'online' }).eq('id', latestDriver.id);
        }
  ...
  const handleBroadcast = async (data: any) => {
      ...
      try {
          if (isShuttle) {
             const { error: updError } = await supabase.from('unihub_drivers').update({ 
                 walletBalance: latestDriver.walletBalance - commissionAmount,
                 status: 'busy' // Broadcasting shuttles are busy
             }).eq('id', latestDriver.id);
             ...
             // Set Pragia/Taxi broadcast to busy too
             await supabase.from('unihub_drivers').update({ status: 'busy' }).eq('id', latestDriver.id);
          }
          ...
          if(insError) {
             ...
             if (isShuttle) {
                ...
                await supabase.from('unihub_drivers').update({ 
  ...
  const handleStartBroadcast = async (nodeId: string) => {
    ...
    // Shuttles prepay, so they don't pay on start
    if (!isShuttle) {
        if (latestDriver.walletBalance < totalCommission) {
        ...
        <div className="max-w-6xl mx-auto space-y-6 lg:space-y-8">
          ...
          {viewMode === 'driver' && (
            <DriverPortal 
              ...
              onReportNoShow={reportPassengerNoShow}
            />
          )}
          {viewMode === 'admin' && (
            !isAdminAuthenticated ? (
              ...
              <AdminPortal 
                ...
                refundRequests={refundRequests} // PASS REFUNDS
                ...
                onApproveRefund={approveRefund} // PASS HANDLER
                onRejectRefund={rejectRefund} // PASS HANDLER
                ...
                onForceResetDriver={forceResetDriver}
              />
            )
          )}
        </div>
