...
const App: React.FC = () => {
  ...
  console.log('🔍 Driver Auth Attempt:', { driverId, pinLength: pin?.length });
  ...
  try {
    ...
    if (driver.pin !== pin) {
      console.error('❌ PIN mismatch:', { 
        entered: pin, 
        stored: driver.pin ? '[REDACTED]' : null 
      });
    ...
    // Check if driver is online (not offline)
    if (driver.status === "offline") {
      console.error('❌ Driver offline:', { status: driver.status });
      alert("Your account is currently offline. Please contact support.");
      setIsSyncing(false);
      return;
    }
    ...
    setSession({
      user: { id: driver.id, email: driver.contact },
