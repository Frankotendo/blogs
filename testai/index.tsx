...
import { ForgotPasswordModal, PassengerTrackingModal } from "./SecurityComponentsFixed";
...
const GlobalVoiceOrb = ({
    ...
    if (isActive) {
      ...
      if (sessionRef.current) {
        sessionRef.current
          ...
          .catch((err: any) => console.error("Failed to close session:", err));
      }
      ...
      tools = [
        {
          name: "update_status",
          description:
            "Update the driver availability status (online, busy, offline).",
          parameters: {
            type: Type.OBJECT,
            properties: {
              status: {
        ...
        },
        {
          ...
          parameters: {
            ...
            properties: {
              location: {
                type: Type.STRING,
                description: 'Location keyword like "Casford" or "Science".',
              },
            },
          },
        },
      ...
      tools = [
        {
          ...
          description:
            "Scans system logs for high-frequency requests, bot patterns, and potential attacks.",
        },
        {
          name: "get_revenue_report",
          description: "Get the total hub revenue and financial status.",
        },
        {
          name: "system_health_check",
          description:
            "Get count of active users, drivers, and pending requests.",
        },
      ];
    } else if (mode === "public") {
      systemInstruction = `${ghanaianPersona}
      ...
      tools = [
        {
          ...
          description:
            "Fill the login/signup form for the user. Call this even with partial info.",
          parameters: {
            type: Type.OBJECT,
            properties: {
      ...
      tools = [
        {
          ...
          parameters: {
            ...
            properties: {
              origin: {
                type: Type.STRING,
                description: "Pickup point (optional)",
              },
              destination: { type: Type.STRING, description: "Dropoff point" },
              vehicleType: {
                type: Type.STRING,
                enum: ["Pragia", "Taxi", "Shuttle"],
              },
              isSolo: {
                type: Type.BOOLEAN,
                description: "True for solo/express ride, False for pool.",
              },
            },
        ...
        },
        {
          ...
          description:
            'Use Google Maps to find a place or business name when the user is unsure of the address or asks for a recommendation (e.g., "Find food", "Take me to Casford").',
          parameters: {
            type: Type.OBJECT,
            properties: {
              query: {
                type: Type.STRING,
                description: "The search query for the place.",
              },
            },
            required: ["query"],
          },
        ...
        {
          name: "confirm_ride",
          description: "Submit the ride request currently on screen.",
        },
        {
          name: "check_pricing",
          description: "Get current fare prices for different vehicle types.",
        },
      ];
    }
            ...
            if (msg.toolCall) {
              ...
              for (const fc of msg.toolCall.functionCalls) {
                ...
                if (fc.name === "update_status" && actions.onUpdateStatus) {
                  ...
                  result = {
                    result: `Balance: ${user?.walletBalance || 0} cedis.`,
                  };
                } else if (fc.name === "scan_for_rides") {
                  const loc = (fc.args as any).location?.toLowerCase();
                  const rides = contextData.nodes
                    .filter((n) => {
                      if (!loc) return true;
                      return (
                        n.origin.toLowerCase().includes(loc) ||
                        n.destination.toLowerCase().includes(loc)
                      );
                    })
                    .slice(0, 3);
                  if (rides.length === 0)
                    result = { result: "No rides found." };
                  else
                    result = {
                      result:
                        `Found ${rides.length} rides. ` +
                        rides.map((r) => r.destination).join(", "),
                    };
                } else if (
                  fc.name === "find_destination" &&
                  actions.onFillRideForm
                ) {
                  const query = (fc.args as any).query;
                  // Use standard GenerateContent with Maps tool for retrieval
                  try {
                    const searchAi = new GoogleGenAI({
                    ...
                    const chunks =
                      mapsResp.candidates?.[0]?.groundingMetadata
                        ?.groundingChunks || [];
                    const mapChunk = chunks.find(
                      (c: any) => c.web?.title || c.web?.uri,
                    ); // Maps grounding structure fallback
                    ...
                    placeName = placeName
                      .replace("The best match is ", "")
                      .replace(".", "");

                    actions.onFillRideForm({ destination: placeName });
                    result = {
                      result: `Found ${placeName}. Ride form updated.`,
                    };
                  } catch (err) {
                    console.error("Maps grounding failed", err);
                    result = {
                      result: "Could not find location data. Please try again.",
                    };
                  }
                  ...
                  const total =
                    contextData.transactions?.reduce(
                      (a, b) => a + b.amount,
                      0,
                    ) || 0;
                  result = {
                    result: `Total Hub Revenue is ${total.toFixed(2)} cedis.`,
                  };
                } else if (fc.name === "system_health_check") {
                  result = {
                    result: `Active Drivers: ${contextData.drivers.length}. Total Rides: ${contextData.nodes.length}. Pending: ${contextData.pendingRequests}.`,
                  };
                } else if (
                  fc.name === "fill_ride_form" &&
                  actions.onFillRideForm
                ) {
                  ...
                  if (safeArgs.destination || safeArgs.origin) {
                    ...
                    result = {
                      result: `Form updated. Destination: ${safeArgs.destination || "Unset"}, Origin: ${safeArgs.origin || "Unset"}. Ask for missing details.`,
                    };
                  } else {
                    result = { result: "No location data found in request." };
                  }
                } else if (
                  fc.name === "confirm_ride" &&
                  actions.onConfirmRide
                ) {
                  actions.onConfirmRide();
                  result = { result: "Ride confirmed and requested." };
                } else if (
                  fc.name === "fill_auth_details" &&
                  actions.onFillAuth
                ) {
                  const safeArgs = cleanArgs(fc.args);
                  actions.onFillAuth(safeArgs);
                  result = {
                    result: `Auth form updated with provided details.`,
                  };
                } else if (fc.name === "check_pricing") {
                  result = {
                    result: `Pragia: ${contextData.settings.farePerPragia}. Taxi: ${contextData.settings.farePerTaxi}.`,
                  };
                }
  ...
  return (
    <>
      ...
      >
        <i
          className={`fas ${isActive ? "fa-microphone-slash" : "fa-microphone"} text-white text-2xl`}
        ></i>
      </button>
      ...
      {isActive && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[450] flex flex-col items-center justify-center animate-in fade-in duration-300">
          ...
          <div className="mt-8 text-center px-4">
            ...
            <p
              className="text-xs font-bold opacity-70 uppercase mt-2 tracking-[0.2em]"
              style={{ color: mode === "admin" ? "#f43f5e" : "#94a3b8" }}
            >
              {mode === "admin"
          ...
          <button
            onClick={toggleSession}
            className="mt-12 px-8 py-3 bg-white/10 rounded-full text-white font-black uppercase text-xs hover:bg-white/20 transition-all"
          >
            End Call
          </button>
        </div>
...
const InlineAd = ({
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          // Ad blocker or tracking prevention; ignore
        }
      }, 500);
    } catch (_e) {
      // AdSense script blocked; do not break app
    }
  }, [
    settings.adSenseStatus,
    settings.adSenseClientId,
    settings.adSenseSlotId,
  ]);
  ...
  return (
    <div
      className={`glass p-4 rounded-[2rem] border border-white/5 flex flex-col items-center justify-center bg-white/5 overflow-hidden ${className}`}
    >
      <p className="text-[8px] font-black uppercase text-slate-500 mb-2 tracking-widest">
        Sponsored
      </p>
...
/** Alias used across the app; same as InlineAd. */
const AdBanner = InlineAd;
      ...
      <div className="glass-bright w-full max-w-sm p-6 rounded-[2.5rem] border border-white/10 text-center relative overflow-hidden animate-in zoom-in">
        ...
        <div className="mb-4">
          ...
          <p className="text-[10px] text-slate-400 mt-1">
            Watch this ad to unlock premium features.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl overflow-hidden min-h-[250px] flex items-center justify-center mb-6 relative">
          ...
          {settings.adSenseStatus === "active" && settings.adSenseClientId && (
            <div
              className="relative z-10 w-full flex justify-center bg-white"
              ref={adRef}
            >
              <ins
                className="adsbygoogle"
...
const HubGateway = ({
  ...
  setShowForgotPin: React.Dispatch<React.SetStateAction<boolean>>;
}) => {
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#020617] p-4 relative overflow-hidden">
      ...
      <div className="glass-bright w-full max-md:mt-12 max-w-md p-8 rounded-[3rem] border border-white/10 relative z-10 animate-in zoom-in duration-500">
        ...
        <div className="mt-6 text-center space-y-3">
          <button
            onClick={() =>
              setFormState({
                ...formState,
                mode: formState.mode === "login" ? "signup" : "login",
              })
            }
          ...
          {formState.mode === "login" && (
            <button
              ...
              className="text-[10px] font-black text-amber-500 uppercase tracking-widest hover:text-amber-400 transition-colors block w-full"
            >
              Forgot PIN?
            </button>
          )}
        ...
        <AdBanner settings={settings} className="mt-6 w-full" />
      </div>
    </div>
  );
};
...
const NavItem = ({ active, icon, label, onClick, badge }: any) => (
  ...
  >
    <div className="flex items-center gap-4">
      <i
        className={`fas ${icon} text-lg w-6 text-center ${active ? "text-white" : "group-hover:scale-110 transition-transform"}`}
      ></i>
      <span className="text-sm font-bold">{label}</span>
    </div>
    {badge && (
      <span className="px-2 py-0.5 bg-rose-500 text-white text-[9px] font-black rounded-full">
        {badge}
      </span>
    )}
  </button>
...
const MobileNavItem = ({ active, icon, label, onClick, badge }: any) => (
  ...
  >
    <div
      className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${active ? "bg-indigo-600 text-white shadow-lg translate-y-[-10px]" : "bg-transparent"}`}
    >
      <i className={`fas ${icon} text-lg`}></i>
    </div>
    <span
      className={`text-[9px] font-black uppercase tracking-wide ${active ? "opacity-100" : "opacity-60"}`}
    >
      {label}
    </span>
    {badge && (
      <span className="absolute top-0 right-0 w-3 h-3 bg-rose-500 border border-[#020617] rounded-full"></span>
    )}
  </button>
);
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          {["All", "Pragia", "Taxi", "Shuttle"].map((type) => (
            <button
              key={type}
              onClick={() =>
                setSearchConfig({ ...searchConfig, vehicleType: type as any })
              }
...
const PassengerPortal = ({
  ...
  const filteredNodes = nodes.filter((n: RideNode) => {
    if (
      ...
      return false;
    if (
      searchConfig.vehicleType !== "All" &&
      n.vehicleType !== searchConfig.vehicleType
    )
      return false;
    return true;
  });

  const myRides = nodes.filter(
    (n: RideNode) => myRideIds.includes(n.id) && n.status !== "completed",
  );
  const availableRides = filteredNodes.filter(
    (n: RideNode) =>
      n.status !== "completed" &&
      n.status !== "dispatched" &&
  ...
  useEffect(() => {
    let base =
      newNode.vehicleType === "Taxi"
        ? settings.farePerTaxi
        : settings.farePerPragia;
  ...
  const handleSubmit = () => {
    if (!newNode.origin || !newNode.destination)
      return alert("Please fill all fields");
    const finalFare = customOffer
      ? parseFloat(customOffer.toString())
      : fareEstimate;
  ...
  if (createMode) {
    return (
      <div className="glass p-4 sm:p-6 md:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-white/10 animate-in zoom-in max-w-lg mx-auto relative">
        {showSoloAd && (
          <AdGate
            onUnlock={handleSoloUnlock}
            label="Unlock Solo Ride Mode"
            settings={settings}
          />
        )}
        ...
        <div className="flex justify-between items-center mb-4 sm:mb-6">
          ...
          <button
            ...
            className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/5 flex items-center justify-center text-white"
          >
        ...
        <div className="space-y-3 sm:space-y-4">
          <div className="grid grid-cols-2 gap-2 p-1 bg-white/5 rounded-2xl">
            <button
              onClick={() => setNewNode({ ...newNode, isSolo: false })}
              className={`py-2.5 sm:py-3 rounded-xl font-black text-[9px] sm:text-[10px] uppercase transition-all ${!newNode.isSolo ? "bg-indigo-600 text-white" : "text-slate-500"}`}
            >
              Pool (Cheaper)
            </button>
            <button
              onClick={toggleSolo}
              className={`py-2.5 sm:py-3 rounded-xl font-black text-[9px] sm:text-[10px] uppercase transition-all flex items-center justify-center gap-1.5 sm:gap-2 ${newNode.isSolo ? "bg-indigo-600 text-white" : "text-slate-500"}`}
            >
              Solo (Express)
          ...
          <div className="relative">
            <input
              value={newNode.origin}
              onChange={(e) =>
                setNewNode({ ...newNode, origin: e.target.value })
              }
              placeholder="Pickup Location"
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-3 sm:pl-4 pr-9 sm:pr-10 py-2.5 sm:py-3 text-white font-bold outline-none text-xs sm:text-sm focus:border-indigo-500"
            />
            <button
              onClick={onTriggerVoice}
              className="absolute right-1.5 sm:right-2 top-1.5 sm:top-2 w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center text-indigo-400 hover:text-white"
            >
          ...
          <div className="relative">
            <input
              value={newNode.destination}
              onChange={(e) =>
                setNewNode({ ...newNode, destination: e.target.value })
              }
          ...
          <div className="flex gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar py-1.5 sm:py-2">
            {["Pragia", "Taxi", "Shuttle"].map((v) => (
              <button
                key={v}
                onClick={() =>
                  setNewNode({ ...newNode, vehicleType: v as any })
                }
                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg border border-white/10 font-black text-[9px] sm:text-[10px] uppercase shrink-0 ${newNode.vehicleType === v ? "bg-amber-500 text-[#020617]" : "bg-white/5 text-slate-400"}`}
              >
                {v}
              </button>
            ))}
          ...
          <div className="p-3 sm:p-6 bg-indigo-900/30 rounded-2xl border border-indigo-500/20 space-y-2 sm:space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-[9px] sm:text-[10px] font-black uppercase text-indigo-300">
                Base Fare
              </span>
            ...
            <div className="min-w-0">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-1 gap-0.5 sm:gap-1">
                <label className="text-[9px] sm:text-[10px] font-black uppercase text-white leading-none">
                  Your Offer (₵)
                </label>
                <span className="text-[7px] sm:text-[9px] text-emerald-400 font-bold uppercase leading-none sm:text-right">
                  <span className="sm:hidden">Boost</span>
                  <span className="hidden sm:inline">Boost to attract drivers</span>
                </span>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
                <button
                  onClick={() => adjustOffer(-0.5)}
                  className="w-9 h-9 sm:w-10 sm:h-10 bg-white/5 rounded-lg sm:rounded-xl border border-white/10 flex items-center justify-center text-white hover:bg-white/10 active:scale-95 transition-all shrink-0"
                >
                  <i className="fas fa-minus text-[10px] sm:text-sm"></i>
                </button>
                <input
                  type="number"
                  step="0.5"
                  ...
                  className="flex-1 min-w-0 bg-[#020617]/50 border border-white/10 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-white font-black text-sm sm:text-lg text-center outline-none focus:border-emerald-500 transition-colors"
                />
                <button
                  onClick={() => adjustOffer(0.5)}
                  className="w-9 h-9 sm:w-10 sm:h-10 bg-white/5 rounded-lg sm:rounded-xl border border-white/10 flex items-center justify-center text-white hover:bg-white/10 active:scale-95 transition-all shrink-0"
                >
                  <i className="fas fa-plus text-[10px] sm:text-sm"></i>
                </button>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-3 sm:py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] sm:text-xs uppercase tracking-widest shadow-xl"
          >
            Confirm Request
          </button>
        </div>
  ...
  return (
    <div className="space-y-8">
      {myRides.length > 0 && (
        <div className="space-y-4">
          ...
          {myRides.map((node: RideNode) => {
            const myPassengerInfo = node.passengers.find(
              (p) => p.phone === currentUser.phone,
            );
            const myPin = myPassengerInfo?.verificationCode;
            const assignedDriver = drivers.find(
              (d: Driver) => d.id === node.assignedDriverId,
            );

            return (
              <div
                key={node.id}
                className="glass p-6 rounded-[2rem] border border-indigo-500/30 bg-indigo-900/10 relative overflow-hidden"
              >
                <div className="flex justify-between items-start mb-4 relative z-10">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`px-2 py-1 rounded-md text-[8px] font-black uppercase ${node.status === "qualified" ? "bg-emerald-500 text-[#020617]" : "bg-amber-500 text-[#020617]"}`}
                      >
                        {node.status}
                      </span>
                ...
                {assignedDriver && (
                  <div className="bg-white/5 p-3 rounded-xl mb-4 border border-white/5">
                    <div className="flex items-center gap-3 mb-3">
                      <img
                        src={
                          ...
                          `https://api.dicebear.com/7.x/initials/svg?seed=${assignedDriver.name}`
                        }
                    ...
                    <div className="flex gap-2">
                      <a
                        href={`tel:${assignedDriver.contact}`}
                        className="flex-1 py-2 bg-indigo-600/20 text-indigo-400 rounded-lg text-[9px] font-black uppercase flex items-center justify-center gap-2 hover:bg-indigo-600 hover:text-white transition-all"
                      >
                        <i className="fas fa-phone"></i> Call
                      </a>
                      <a
                        href={`https://wa.me/${assignedDriver.contact.replace(/[^0-9]/g, "")}`}
                        target="_blank"
                        className="flex-1 py-2 bg-emerald-500/20 text-emerald-500 rounded-lg text-[9px] font-black uppercase flex items-center justify-center gap-2 hover:bg-emerald-500 hover:text-[#020617] transition-all"
                      >
                ...
                {node.assignedDriverId && myPin && (
                  <div className="bg-black/30 p-4 rounded-xl mb-4 flex items-center justify-between gap-4">
                    <div>
                      ...
                      <button
                        onClick={() => setExpandedQr(myPin)}
                        className="mt-2 px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-lg text-[9px] font-black uppercase flex items-center gap-2 border border-white/10 transition-colors"
                      >
                    ...
                    >
                      <img
                        src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${myPin}`}
                        className="w-20 h-20"
                        alt="Ride QR"
                      />
                    </div>
                ...
                <div className="flex gap-2">
                  {node.status === "forming" &&
                    node.passengers.length > 1 &&
                    !node.assignedDriverId && (
                      <button
                        onClick={() => onForceQualify(node.id)}
                        className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-black text-[9px] uppercase"
                      >
                        Go Now (Pay Extra)
                      </button>
                    )}
                  ...
                  {node.leaderPhone === currentUser.phone &&
                    !node.assignedDriverId && (
                      <button
                        ...
                        className="w-10 flex items-center justify-center bg-rose-500/10 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-all"
                      >
      ...
      >
        ...
        <h3 className="text-lg font-black uppercase italic text-white group-hover:text-amber-500 transition-colors">
          Start New Trip
        </h3>
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
          Create a pool or go solo
        </p>
      </div>

      <div>
        <h3 className="text-xs font-black uppercase text-slate-500 tracking-widest px-2 mb-4">
          Community Rides
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {availableRides.length === 0 && (
            <p className="text-slate-600 text-xs font-bold uppercase col-span-full text-center py-8">
          ...
          {availableRides.map((node: RideNode, index: number) => {
            const isPartnerOffer =
              node.assignedDriverId &&
              (node.status === "forming" || node.status === "qualified");
            const seatsLeft = Math.max(
              0,
              node.capacityNeeded - node.passengers.length,
            );

            return (
              <React.Fragment key={node.id}>
                <div
                  className={`glass p-6 rounded-[2rem] border transition-all ${isPartnerOffer ? "border-emerald-500/50 shadow-lg shadow-emerald-500/10" : "border-white/5 hover:border-white/10"}`}
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <div className="flex gap-2 items-center mb-1">
                        <span className="px-2 py-1 bg-white/10 rounded-md text-[8px] font-black uppercase text-slate-300">
                          {node.vehicleType}
                        </span>
                        {isPartnerOffer && (
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[9px] font-bold text-slate-500 uppercase">
                        {node.passengers.length}/{node.capacityNeeded} Seats
                      </p>
                    </div>
                  </div>
                    ...
                    {node.capacityNeeded > 5 ? (
                      ...
                      <>
                        {node.passengers.map((p, i) => (
                          <div
                            key={i}
                            className="w-6 h-6 rounded-full bg-indigo-600 flex items-center justify-center text-[8px] font-bold text-white border border-[#020617]"
                            title={p.name}
                          >
                            {p.name[0]}
                          </div>
      ...
      <AdBanner settings={settings} className="w-full max-w-md mx-auto" />

      {expandedQr && (
        <div
          className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-center justify-center p-6"
          onClick={() => setExpandedQr(null)}
        >
          <div
            className="bg-white p-8 rounded-[2.5rem] w-full max-w-sm text-center animate-in zoom-in relative"
          ...
          >
            ...
            <h3 className="text-2xl font-black uppercase text-[#020617] mb-2">
              Scan Me
            </h3>
            ...
            <div className="bg-[#020617] p-2 rounded-2xl inline-block mb-6 shadow-2xl">
              <div className="bg-white p-2 rounded-xl">
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${expandedQr}`}
            ...
            <div className="mb-6">
              ...
              <p className="text-5xl font-black text-[#020617] tracking-[0.5em]">
                {expandedQr}
              </p>
            </div>
...
const AdminLogin = ({ onLogin, showAdminForgotPassword, setShowAdminForgotPassword }: any) => {
  ...
  return (
    <div className="max-w-sm mx-auto glass p-8 rounded-[2.5rem] border border-white/10 text-center space-y-6 animate-in zoom-in">
      ...
      <input
        ...
        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-bold outline-none text-xs focus:border-rose-500"
      />
      <input
        type="password"
        value={pass}
      ...
      <button
        ...
        className="w-full py-4 bg-rose-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-xl"
      >
        Authenticate
      </button>
...
const AdminForgotPassword = ({ onClose, onResetPassword }: any) => {
  const [email, setEmail] = useState("");
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const { data, error } = await supabase.rpc('get_security_questions_text');
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.rpc('check_admin_security_setup', { p_admin_email: email });
      if (error) throw error;

      if (!data.has_security_questions) {
        setError("No security questions set up for this admin account.");
        return;
      }
  ...
  const handleSecurityQuestionsSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.rpc('reset_admin_password_with_security_questions', {
        p_admin_email: email,
        p_questions: selectedQuestions,
        p_answers: answers,
        p_new_password: "temp_password" // Will be replaced in next step
      });
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="max-w-md w-full glass p-8 rounded-[2.5rem] border border-white/10 text-center space-y-6 animate-in zoom-in">
        <div className="w-16 h-16 bg-amber-600 rounded-2xl mx-auto flex items-center justify-center text-white shadow-xl shadow-amber-900/20">
          ...
          <>
            ...
            <form onSubmit={handleEmailSubmit} className="space-y-4">
              <input
                ...
                onChange={(e) => setEmail(e.target.value)}
                placeholder="Admin Email"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-bold outline-none text-xs focus:border-amber-500"
                required
              />
              ...
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={onClose}
                  className="flex-1 py-3 bg-white/10 text-white rounded-xl font-black text-xs uppercase tracking-widest"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 py-3 bg-amber-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-xl disabled:opacity-50"
                >
                ...
                <div key={index} className="space-y-2">
                  ...
                  >
                    ...
                    {securityQuestions.map((q, i) => (
                      <option key={i} value={q} className="bg-gray-800">
                        {q}
                      </option>
                    ))}
                  </select>
        ...
        {step === 4 && (
          <>
            <div className="w-16 h-16 bg-green-600 rounded-2xl mx-auto flex items-center justify-center text-white shadow-xl shadow-green-900/20">
            ...
            <button
              onClick={onClose}
              className="w-full py-3 bg-green-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-xl"
            >
              Return to Login
            </button>
          </>
...
const AdminSecuritySetup = ({ onClose, adminEmail }: any) => {
  const [securityQuestions, setSecurityQuestions] = useState<string[]>([]);
  const [selectedQuestions, setSelectedQuestions] = useState<string[]>([]);
  const [answers, setAnswers] = useState<string[]>(["", "", ""]);
  const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedQuestions.some(q => !q) || answers.some(a => !a)) {
      setError("Please select 3 questions and provide answers for all.");
      setLoading(false);
      return;
    }

    try {
      const { data, error } = await supabase.rpc('setup_admin_security_questions', {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="max-w-md w-full glass p-8 rounded-[2.5rem] border border-white/10 text-center space-y-6 animate-in zoom-in">
        <div className="w-16 h-16 bg-indigo-600 rounded-2xl mx-auto flex items-center justify-center text-white shadow-xl shadow-indigo-900/20">
          <i className="fas fa-shield-halved text-2xl"></i>
        </div>
        <div>
          <h2 className="text-xl font-black italic uppercase text-white">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {[0, 1, 2].map((index) => (
            <div key={index} className="space-y-2">
              <select
                value={selectedQuestions[index] || ""}
                onChange={(e) => handleQuestionSelect(index, e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-bold outline-none text-xs focus:border-indigo-500"
                required
              >
                <option value="" disabled>Select Question {index + 1}</option>
                {securityQuestions.map((q, i) => (
          ...
          <div className="flex gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-xl disabled:opacity-50"
            >
              {loading ? "Setting Up..." : "Set Up Security"}
            </button>
          </div>
        </form>
      </div>
  ...
  <div className="glass p-6 rounded-[2rem] border border-white/5">
    ...
    <ul className="space-y-2">
      {points.map((p: string, i: number) => (
        <li
          key={i}
          className="text-[10px] text-slate-400 font-medium leading-relaxed flex gap-2"
        >
          <span
            className={`mt-1 w-1 h-1 rounded-full shrink-0 ${color.replace("text-", "bg-")}`}
          ></span>
          {p}
        </li>
      ))}
...
const AiHelpDesk = ({ onClose, settings }: any) => {
  ...
  >([
    {
      role: "model",
      text: `Hello! I'm the NexRyde AI Assistant. I can help you with app features, pricing, and safety tips. How can I help you today?`,
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "model", text: text || "I didn't catch that." },
      ]);
    } catch (err) {
      setMessages((prev) => [
        ...prev,
      ...
      <div className="bg-[#020617] w-full max-w-lg h-[80vh] sm:h-[600px] sm:rounded-[2.5rem] flex flex-col border border-white/10 shadow-2xl relative animate-in slide-in-from-bottom">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4" ref={scrollRef}>
          {messages.map((m, i) => (
            ...
            >
              <div
                className={`max-w-[80%] p-4 rounded-2xl text-xs font-medium leading-relaxed ${m.role === "user" ? "bg-indigo-600 text-white rounded-br-none" : "bg-white/10 text-slate-200 rounded-bl-none"}`}
              >
                {m.text}
              </div>
            </div>
        ...
        <div className="shrink-0 p-3 sm:p-4 border-t border-white/5">
          <AdBanner settings={settings} className="mb-3 w-full max-w-[300px] mx-auto" />
          <div className="flex gap-2 bg-white/5 p-2 rounded-[1.5rem] border border-white/5 focus-within:border-indigo-500/50 transition-colors">
            <input
              ...
              className="flex-1 min-w-0 bg-transparent px-4 text-white text-sm outline-none placeholder:text-slate-600"
              placeholder="Ask about rides, prices..."
            />
            <button
              onClick={handleSend}
              disabled={loading || !input.trim()}
              className="w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 transition-transform"
            >
...
const DriverPortal = ({
  ...
  isLoading,
  showDriverForgotPin,
  setShowDriverForgotPin,
  onShowPassengerTracking,
  ...
  console.log('DriverPortal mode:', { regMode, hasActiveDriver: !!activeDriver });

  const [activeTab, setActiveTab] = useState<
    "market" | "active" | "wallet" | "broadcast" | "tracking"
  >("market");
  ...
  const myActiveRides = dispatchedNodes.filter(
    (n: any) =>
      n.assignedDriverId === activeDriver?.id && n.status !== "completed",
  );
  const availableRides = qualifiedNodes.filter((n: any) => {
    if (activeDriver && n.vehicleType !== activeDriver.vehicleType)
      return false;
    if (
      searchConfig.query &&
      !n.origin.toLowerCase().includes(searchConfig.query.toLowerCase()) &&
  ...
  const myBroadcasts = allNodes.filter(
    (n: any) =>
      n.assignedDriverId === activeDriver?.id && n.status === "forming",
  );
  ...
  const estimatedCapacity =
    parseInt(broadcastData.seats) || (isShuttle ? 10 : 3);
  const commissionRate = isShuttle
    ? settings.shuttleCommission || 0
    : settings.commissionPerSeat;
  const requiredBalanceForBroadcast = isShuttle
    ? estimatedCapacity * commissionRate
    : 0;
  const canAffordBroadcast = activeDriver
    ? activeDriver.walletBalance >= requiredBalanceForBroadcast
    : false;
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <select
                ...
                onChange={(e) =>
                  setRegData({ ...regData, vehicleType: e.target.value })
                }
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-bold outline-none text-xs"
              >
              ...
              <input
                ...
                onChange={(e) =>
                  setRegData({ ...regData, licensePlate: e.target.value })
                }
            ...
            <div className="p-4 bg-indigo-600/10 rounded-xl border border-indigo-600/20">
              <p className="text-[10px] font-bold text-indigo-400 uppercase mb-2">
                Registration Fee: ₵{settings.registrationFee}
              </p>
              <p className="text-[9px] text-slate-400">
                Send to <b>{settings.adminMomo}</b> ({settings.adminMomoName})
                and enter Ref ID below.
              </p>
              <input
                value={regData.momoReference}
                onChange={(e) =>
                  setRegData({
                    ...regData,
                    momoReference: e.target.value,
                    amount: settings.registrationFee,
                  })
                }
                placeholder="MoMo Reference ID"
                className="mt-2 w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-bold outline-none text-xs"
              />
            </div>
      ...
      <div className="glass p-8 rounded-[2.5rem] border border-white/10 max-w-sm mx-auto text-center space-y-6 animate-in zoom-in">
        ...
        <div className="pt-4 border-t border-white/5 space-y-3">
          <button
            ...
            className="text-[9px] font-black text-slate-500 uppercase hover:text-white transition-colors block w-full"
          >
            Join the Fleet
          </button>
          
          <button
            onClick={() => onShowForgotPassword('driver')}
            className="text-[9px] font-black text-amber-500 uppercase hover:text-amber-400 transition-colors block w-full"
          >
            Forgot Password?
          </button>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 overflow-x-auto no-scrollbar">
          {["market", "active", "broadcast", "wallet", "tracking"].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab as any)}
              className={`flex-1 min-w-[80px] px-4 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all relative ${activeTab === tab ? "bg-indigo-600 text-white shadow-lg" : "text-slate-500 hover:text-white"}`}
            >
              {tab === "tracking" ? (
                <>
        ...
        <button
          ...
          className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-emerald-500 to-teal-500 flex items-center justify-center text-[#020617] shadow-lg hover:scale-105 transition-transform disabled:opacity-50"
        >
          {isPlayingBriefing ? (
              ...
              <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
                {missions.map((m: any) => {
                  ...
                  return (
                    <div
                      key={m.id}
                      className={`min-w-[250px] p-6 rounded-[2rem] border relative overflow-hidden ${joined ? "bg-emerald-500 text-[#020617] border-emerald-400" : "glass border-white/10"}`}
                    >
                      <div className="relative z-10">
                        ...
                        <p
                          className={`text-[10px] font-bold uppercase ${joined ? "text-[#020617]/70" : "text-indigo-400"}`}
                        >
                          Fee: ₵{m.entryFee}
                        </p>
                        <p
                          className={`text-xs mt-2 ${joined ? "text-[#020617]" : "text-slate-400"}`}
                        >
                          {m.description}
                        </p>
                        <div className="mt-4 flex justify-between items-center">
                          <span
                            className={`text-[9px] font-black uppercase ${joined ? "text-[#020617]/50" : "text-slate-500"}`}
                          >
                            {m.driversJoined.length} Partners
                          </span>
                          {!joined && (
                            <button
                              onClick={() =>
                                onJoinMission(m.id, activeDriver.id)
                              }
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {availableRides.map((node: any) => {
                ...
                const baseFare =
                  node.vehicleType === "Taxi"
                    ? settings.farePerTaxi
                    : settings.farePerPragia;
                const expectedFare = node.isSolo
                  ? baseFare * settings.soloMultiplier
                  : baseFare;
                ...
                return (
                  <div
                    key={node.id}
                    className={`glass p-6 rounded-[2rem] border transition-all group relative ${isHighFare ? "border-amber-500/40 shadow-lg shadow-amber-500/10" : "border-white/5 hover:border-indigo-500/30"}`}
                  >
                    {node.isSolo && (
                    ...
                    <div className="flex justify-between items-center mb-4 p-3 bg-white/5 rounded-xl border border-white/5">
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <p
                            className={`text-lg font-black ${isHighFare ? "text-amber-400" : "text-white"}`}
                          >
                            ₵ {node.farePerPerson}
                          </p>
                          {isHighFare && (
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-lg font-black text-emerald-400">
                          ₵{" "}
                          {(
                            node.farePerPerson * (isSolo ? 1 : capacity)
                          ).toFixed(2)}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-1 mb-4">
                      {[...Array(capacity)].map((_, i) => (
                        <div
                          key={i}
                          className={`h-1.5 flex-1 rounded-full ${i < paxCount ? "bg-indigo-500" : "bg-white/10"}`}
                        ></div>
                      ))}
                    </div>
                    <button
                      onClick={() => onAccept(node.id, activeDriver.id)}
                      disabled={!canAccept}
                      className="w-full py-3 bg-white text-[#020617] rounded-xl font-black text-[10px] uppercase hover:bg-indigo-600 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Accept Request
                    </button>
                  </div>
      ...
      {activeTab === "active" && (
        <div className="space-y-4">
          ...
          {myActiveRides.map((node: any) => (
            <div
              key={node.id}
              className="glass p-8 rounded-[2.5rem] border border-indigo-500/30 bg-indigo-900/10 relative overflow-hidden"
            >
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="px-3 py-1 bg-emerald-500 text-[#020617] rounded-lg text-[9px] font-black uppercase mb-2 inline-block animate-pulse">
                      In Progress
                    </span>
                  ...
                  <div className="text-right">
                    <p className="text-3xl font-black text-white">
                      ₵{" "}
                      {node.negotiatedTotalFare ||
                        node.farePerPerson * node.passengers.length}
                    </p>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <div className="flex gap-3 mt-1">
                                <a
                                  href={`tel:${p.phone}`}
                                  className="text-[10px] text-indigo-400 font-bold flex items-center gap-1 hover:text-white"
                                >
                                  <i className="fas fa-phone"></i> Call
                                </a>
                                <a
                                  ...
                                  target="_blank"
                                  className="text-[10px] text-emerald-400 font-bold flex items-center gap-1 hover:text-white"
                                >
                ...
                <button
                  onClick={() => setIsScanning(node.id)}
                  className="w-full py-8 bg-gradient-to-tr from-white to-slate-200 text-indigo-900 rounded-[2rem] shadow-2xl flex flex-col items-center justify-center mb-6 animate-pulse hover:scale-[1.02] transition-transform"
                >
                ...
                <div className="flex justify-center mb-6">
                  <button
                    ...
                    className="text-[10px] font-bold text-slate-400 underline uppercase"
                  >
                ...
                {showManualEntry && (
                  <div className="flex flex-col sm:flex-row gap-2 mb-6 animate-in slide-in-from-top-2 fade-in">
                    <input
                      type="number"
                      placeholder="Enter PIN manually"
                      className="w-full sm:flex-[2] bg-white text-[#020617] rounded-xl px-4 text-center font-black text-lg outline-none placeholder:text-slate-400 placeholder:text-xs placeholder:font-bold"
                      value={verifyCode}
                      onChange={(e) => setVerifyCode(e.target.value)}
                    />
                    <button
                      onClick={() => {
                        onVerify(node.id, verifyCode);
                        setVerifyCode("");
                      }}
                      className="w-full sm:flex-1 py-4 bg-emerald-500 text-[#020617] rounded-xl font-black text-[10px] uppercase shadow-lg"
                    >
                      Verify
                    </button>
                  </div>
                ...
                <div className="text-center">
                  <button
                    onClick={() => onCancel(node.id)}
                    className="w-12 h-12 mx-auto flex items-center justify-center bg-rose-500/20 text-rose-500 rounded-full hover:bg-rose-500 hover:text-white transition-all"
                  >
                    <i className="fas fa-ban"></i>
                  </button>
                  <p className="text-[9px] text-rose-500 font-bold uppercase mt-2">
                    Cancel Trip
                  </p>
                </div>
              </div>
      ...
      {activeTab === "broadcast" && (
        <div className="space-y-8">
          <div className="glass p-6 md:p-8 rounded-[2.5rem] border border-white/10">
            ...
            <div className="space-y-4">
              <input
                value={broadcastData.origin}
                onChange={(e) =>
                  setBroadcastData({ ...broadcastData, origin: e.target.value })
                }
                placeholder="Starting Point"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-bold outline-none text-xs"
              />
              <input
                value={broadcastData.destination}
                onChange={(e) =>
                  setBroadcastData({
                    ...broadcastData,
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[8px] text-slate-500 uppercase font-bold pl-2">
                    Fare (₵)
                  </label>
                  <input
                    type="number"
                    value={broadcastData.fare}
                    onChange={(e) =>
                      setBroadcastData({
                        ...broadcastData,
              ...
              <input
                value={broadcastData.note}
                onChange={(e) =>
                  setBroadcastData({ ...broadcastData, note: e.target.value })
                }
                placeholder="Note (e.g. Leaving in 5 mins)"
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white font-bold outline-none text-xs"
              />
              ...
              {isShuttle && (
                <div className="p-4 bg-rose-500/10 rounded-xl border border-rose-500/20 text-center">
                  ...
                  <p className="text-[9px] text-slate-400">
                    Shuttle commission (₵
                    {(estimatedCapacity * commissionRate).toFixed(2)}) is
          ...
          {myBroadcasts.length > 0 && (
            <div className="space-y-4">
              ...
              {myBroadcasts.map((node: any) => (
                <div
                  key={node.id}
                  className="glass p-6 rounded-[2rem] border border-indigo-500/30"
                >
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-[9px] text-slate-500 uppercase">
                        {node.passengers.length}/{node.capacityNeeded} Joined
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2 mb-4">
                    {node.passengers.map((p: any) => (
                      <div
                        key={p.id}
                        className="flex items-center gap-2 text-xs text-slate-300 bg-white/5 p-2 rounded-lg"
                      >
                        <i className="fas fa-user"></i> {p.name}
                      </div>
                    ))}
                    {node.passengers.length === 0 && (
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => onStartBroadcast(node.id)}
                      disabled={node.passengers.length === 0}
                      className="flex-[2] py-3 bg-emerald-500 text-[#020617] rounded-xl font-black text-[9px] uppercase disabled:opacity-50"
                    >
                      Start Trip
                    </button>
                    <button
                      onClick={() => onCancel(node.id)}
                      className="flex-1 py-3 bg-rose-500/20 text-rose-500 rounded-xl font-black text-[9px] uppercase"
                    >
                      Cancel
                    </button>
                  </div>
          ...
          <div className="p-6 bg-white/5 rounded-[2rem] border border-white/5 space-y-4">
            ...
            <div className="space-y-3">
              <input
                ...
                placeholder="Amount (₵)"
                id="topup-amount"
                className="w-full bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-white font-bold outline-none text-xs"
              />
              <input
                placeholder="MoMo Reference ID"
                id="topup-ref"
              ...
              <button
                onClick={() => {
                  const amt = (
                    document.getElementById("topup-amount") as HTMLInputElement
                  ).value;
                  const ref = (
                    document.getElementById("topup-ref") as HTMLInputElement
                  ).value;
                  onRequestTopup(activeDriver.id, parseFloat(amt), ref);
                }}
                className="w-full py-3 bg-emerald-500 text-[#020617] rounded-xl font-black text-[9px] uppercase shadow-lg"
              >
                Request Credit
              </button>
            </div>
            <p className="text-[9px] text-slate-500 text-center">
              Admin: {settings.adminMomo} ({settings.adminMomoName})
            </p>
          </div>
          ...
          <div className="pt-4 border-t border-white/5">
            <div className="flex justify-between items-center mb-4">
              ...
              <select
                ...
                className="bg-white/5 text-white text-[10px] font-bold uppercase rounded-lg px-2 py-1 outline-none border border-white/10"
              >
      ...
      {activeTab === "tracking" && (
        <div className="glass p-8 rounded-[2.5rem] border border-white/10 space-y-6">
          <div className="text-center">
            <div className="w-16 h-16 bg-blue-600 rounded-2xl mx-auto flex items-center justify-center text-white shadow-xl shadow-blue-600/20 mb-4">
          ...
          <div className="space-y-4">
            <button
              onClick={onShowPassengerTracking}
              className="w-full py-4 bg-blue-600 text-white rounded-xl font-black text-sm uppercase shadow-lg hover:bg-blue-500 transition-all flex items-center justify-center gap-3"
            >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
                <i className="fas fa-route text-blue-400 text-2xl mb-2"></i>
                <p className="text-white font-bold text-sm">Active Routes</p>
                <p className="text-blue-400 text-2xl font-black">{myActiveRides.length}</p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 text-center">
                <i className="fas fa-users text-green-400 text-2xl mb-2"></i>
                <p className="text-white font-bold text-sm">Total Passengers</p>
                <p className="text-green-400 text-2xl font-black">
                  {myActiveRides.reduce((sum, ride) => sum + (ride.passengers?.length || 0), 0)}
                </p>
              </div>
            </div>
...
const AdminPortal = ({
  ...
  const handleUpdateCredentials = async () => {
    if (!newAdminEmail && !newAdminPassword)
      return alert("Enter a new email or password to update.");
    ...
    if (error) alert("Update failed: " + error.message);
      ...
      alert(
        "Credentials updated! Please login again if you changed your password.",
      );
      setNewAdminEmail("");
      setNewAdminPassword("");
    }
  ...
  const handlePortfolioUpload = async (e: any) => {
    ...
    const newImages = await Promise.all(
      files.map((f: any) => compressImage(f)),
    );
    setLocalSettings({
      ...localSettings,
  ...
  const handlePulseAnalysis = async () => {
    ...
    try {
      const activeRides = nodes.filter(
  ...
  return (
    <div className="space-y-6">
      <div className="glass p-6 rounded-[2.5rem] border border-white/10 flex justify-between items-center">
        ...
        <button
          onClick={onLock}
          className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-slate-500 hover:text-rose-500 hover:bg-white/10 transition-all"
        >
      ...
      <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 overflow-x-auto no-scrollbar">
        {[
          ...
          <button
            ...
            className={`flex-1 min-w-[80px] px-4 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? "bg-rose-600 text-white shadow-lg" : "text-slate-500 hover:text-white"}`}
          >
            {tab}
          </button>
        ))}
        ...
        <div className="space-y-6">
          <div className="glass p-6 rounded-[2rem] border border-white/10">
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              <input
                ...
                placeholder="Name"
                className="bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white text-xs font-bold outline-none"
              />
              <input
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              <select
                ...
                className="bg-white/5 border border-white/10 rounded-xl px-2 py-2 text-white text-xs font-bold outline-none"
              >
                <option value="Pragia">Pragia</option>
                <option value="Taxi">Taxi</option>
                <option value="Shuttle">Shuttle</option>
            ...
            <button
              ...
              className="w-full py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl font-black text-[9px] uppercase transition-all"
            >
              Manually Register Driver
            </button>
          </div>
          ...
          <div className="space-y-2">
            {drivers.map((d: any) => (
              <div
                key={d.id}
                className="glass p-4 rounded-2xl flex justify-between items-center border border-white/5"
              >
                <div>
                  <p className="text-sm font-black text-white">{d.name}</p>
                  <p className="text-[10px] text-slate-400 font-bold">
                    {d.vehicleType} • {d.licensePlate}
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  <span
                    className={`text-[9px] font-black uppercase ${d.status === "online" ? "text-emerald-500" : "text-slate-500"}`}
                  >
                    {d.status}
                  </span>
                  <button
      ...
      {activeTab === "finance" && (
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            {topupRequests.filter((r: any) => r.status === "pending").length ===
              0 && (
            ...
            {topupRequests
              ...
              .map((r: any) => (
                <div
                  key={r.id}
                  className="glass p-4 rounded-2xl flex justify-between items-center border border-indigo-500/30"
                >
                  <div>
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => onApproveTopup(r.id)}
                      className="px-4 py-2 bg-emerald-500 text-[#020617] rounded-lg text-[9px] font-black uppercase"
                    >
                      Approve
                    </button>
                    <button
                      onClick={() => onRejectTopup(r.id)}
                      className="px-4 py-2 bg-rose-500/10 text-rose-500 rounded-lg text-[9px] font-black uppercase"
                    >
                      Reject
                    </button>
                  </div>
          ...
          <div className="space-y-2">
            ...
            {registrationRequests.filter((r: any) => r.status === "pending")
              .length === 0 && (
              <p className="text-center text-slate-600 text-xs py-4">
            ...
            {registrationRequests
              ...
              .map((r: any) => (
                <div
                  key={r.id}
                  className="glass p-4 rounded-2xl border border-indigo-500/30"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <p className="text-sm font-black text-white">{r.name}</p>
                      <p className="text-[10px] text-slate-400 font-bold">
                        {r.vehicleType} • {r.contact}
                      </p>
                    </div>
                    <div className="text-right">
                  ...
                  <div className="flex gap-2 mt-2">
                    <button
                      ...
                      className="flex-1 py-2 bg-emerald-500 text-[#020617] rounded-lg text-[9px] font-black uppercase"
                    >
                      Approve Partner
                    </button>
                    <button
                      onClick={() => onRejectRegistration(r.id)}
                      className="flex-1 py-2 bg-rose-500/10 text-rose-500 rounded-lg text-[9px] font-black uppercase"
                    >
                      Reject
                    </button>
                  </div>
        ...
        <div className="glass p-8 rounded-[2.5rem] border border-white/10 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="number"
                value={localSettings.farePerPragia}
                onChange={(e) =>
                  setLocalSettings({
                    ...localSettings,
                    farePerPragia: parseFloat(e.target.value),
                  })
                }
            ...
            <div>
              ...
              <input
                type="number"
                value={localSettings.farePerTaxi}
                onChange={(e) =>
                  setLocalSettings({
                    ...localSettings,
                    farePerTaxi: parseFloat(e.target.value),
                  })
                }
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold"
              />
            ...
            <div>
              ...
              <input
                type="number"
                value={localSettings.commissionPerSeat}
                onChange={(e) =>
                  setLocalSettings({
                    ...localSettings,
                    commissionPerSeat: parseFloat(e.target.value),
                  })
                }
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold"
              />
            ...
            <div>
              ...
              <input
                type="number"
                value={localSettings.shuttleCommission || 0}
                onChange={(e) =>
                  setLocalSettings({
                    ...localSettings,
                    shuttleCommission: parseFloat(e.target.value),
                  })
                }
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold"
              />
            ...
            <div>
              ...
              <input
                type="number"
                step="0.1"
                value={localSettings.soloMultiplier}
                onChange={(e) =>
                  setLocalSettings({
                    ...localSettings,
                    soloMultiplier: parseFloat(e.target.value),
                  })
                }
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold"
              />
          ...
          <div>
            ...
            <input
              value={localSettings.adminMomoName}
              onChange={(e) =>
                setLocalSettings({
                  ...localSettings,
                  adminMomoName: e.target.value,
                })
              }
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold mb-2"
            />
            <label className="text-[9px] font-bold text-slate-500 uppercase">
              Admin MoMo Number
            </label>
            <input
              value={localSettings.adminMomo}
              onChange={(e) =>
                setLocalSettings({
                  ...localSettings,
                  adminMomo: e.target.value,
                })
              }
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold"
            />
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setLocalSettings({
                  ...localSettings,
                  hub_announcement: e.target.value,
                })
              }
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold text-xs h-20"
              placeholder="Broadcast message..."
            />
          </div>

          <div>
            <label className="text-[9px] font-bold text-slate-500 uppercase mb-2 block">
              App Logo
            </label>
            <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <input
                type="file"
                accept="image/*"
                onChange={(e) => handleImageUpload(e, "appWallpaper")}
                className="text-[9px] text-slate-400"
              />
              {localSettings.appWallpaper && (
                <button
                  onClick={() =>
                    setLocalSettings({ ...localSettings, appWallpaper: "" })
                  }
                  className="ml-2 text-[9px] font-bold text-rose-500 uppercase hover:text-white"
                >
                  Clear
                </button>
              )}
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                setLocalSettings({
                  ...localSettings,
                  aboutMeText: e.target.value,
                })
              }
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold text-xs h-24"
            />
          </div>
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setLocalSettings({
                        ...localSettings,
                        aboutMeImages: localSettings.aboutMeImages.filter(
          ...
          <div>
            <h4 className="text-xs font-black uppercase text-white mb-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white font-bold text-xs"
          ...
          <div className="p-4 sm:p-6 rounded-2xl bg-white/5 border border-white/10 min-w-0 overflow-hidden">
            <h4 className="text-xs font-black uppercase text-white mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <div className="min-w-0">
                ...
                <select
                  value={localSettings.adSenseStatus || "inactive"}
                  onChange={(e) =>
                    setLocalSettings({
                      ...localSettings,
                      adSenseStatus: e.target.value as any,
                    })
                  }
                  className="w-full min-w-0 bg-white/5 border border-white/10 rounded-xl px-3 sm:px-4 py-2.5 text-white font-bold text-xs outline-none focus:border-amber-500/50"
                >
              ...
              <div className="min-w-0 sm:col-span-2">
                ...
                <input
                  ...
                  placeholder="ca-pub-7812709042449387"
                  className="w-full min-w-0 bg-white/5 border border-white/10 rounded-xl px-3 sm:px-4 py-2.5 text-white font-bold text-xs outline-none focus:border-amber-500/50 placeholder:text-slate-500"
                />
              </div>
              <div className="min-w-0">
                <label className="block text-[9px] font-bold text-slate-500 uppercase mb-1">
              ...
              <div className="min-w-0">
                ...
                <input
                  value={localSettings.adSenseLayoutKey || ""}
                  onChange={(e) =>
                    setLocalSettings({
                      ...localSettings,
                      adSenseLayoutKey: e.target.value,
                    })
                  }
                  className="w-full min-w-0 bg-white/5 border border-white/10 rounded-xl px-3 sm:px-4 py-2.5 text-white font-bold text-xs outline-none focus:border-amber-500/50"
                />
          ...
          <div className="pt-6 border-t border-white/5">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl font-black text-[9px] uppercase transition-all"
              >
                Update Credentials
              </button>
            </div>
          ...
          <button
            ...
            disabled={isSaving}
            className="w-full py-4 bg-rose-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl disabled:opacity-50 mt-4"
          >
          ...
          <div className="glass p-6 rounded-[2rem] border border-white/10">
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-white text-xs font-bold outline-none"
              />
              <input
              ...
              >
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">
                    {m.driversJoined.length} Drivers Stationed • Fee: ₵
                    {m.entryFee}
                  </p>
                </div>
                <button
      ...
      {activeTab === "rides" && (
        <div className="space-y-4">
          ...
          {nodes.map((n: any) => (
            <div
              key={n.id}
              className="glass p-4 rounded-2xl border border-white/5 relative overflow-hidden"
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span
                      className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${n.status === "completed" ? "bg-emerald-500/20 text-emerald-500" : "bg-amber-500/20 text-amber-500"}`}
                    >
                      {n.status}
                    </span>
              ...
              <div className="flex gap-2 mt-2">
                {n.status !== "completed" && (
                  <>
                    ...
                    <button
                      onClick={() => onSettleRide(n.id)}
                      className="flex-1 py-2 bg-emerald-500/10 text-emerald-500 rounded-lg text-[9px] font-black uppercase hover:bg-emerald-500 hover:text-white transition-all"
                    >
                      Settle
                    </button>
                  </>
...
const App: React.FC = () => {
  ...
  const [newNode, setNewNode] = useState<Partial<RideNode>>({
    origin: "",
    destination: "",
    vehicleType: "Pragia",
    isSolo: false,
  ...
  const [isNewUser, setIsNewUser] = useState(
    () => !localStorage.getItem("nexryde_seen_welcome_v1"),
  );
  const [isSyncing, setIsSyncing] = useState(true);
  const [dismissedAnnouncement, setDismissedAnnouncement] = useState(() =>
    localStorage.getItem("nexryde_dismissed_announcement"),
  );
  const [installPrompt, setInstallPrompt] = useState<any>(null);
  const [showInstallBanner, setShowInstallBanner] = useState(() =>
    !localStorage.getItem("nexryde_install_banner_dismissed"),
  );
  ...
  const [showDriverForgotPin, setShowDriverForgotPin] = useState(false);
  const [showAdminForgotPassword, setShowAdminForgotPassword] = useState(false);
  
  // Security modals
  const [showForgotPassword, setShowForgotPassword] = useState(false);
  const [forgotPasswordType, setForgotPasswordType] = useState<'user' | 'driver' | 'admin'>('user');
  const [showPassengerTracking, setShowPassengerTracking] = useState(false);

  // Debug security modal state
  console.log('Security modal state:', { showForgotPassword, forgotPasswordType, showPassengerTracking });
  ...
  const isVaultAccess = useMemo(() => {
    return (
      new URLSearchParams(window.location.search).get("access") === "vault"
    );
  }, []);
  ...
  const fetchData = async () => {
    ...
    try {
      const [
        ...
        supabase
          .from("unihub_transactions")
          .select("*")
          .order("timestamp", { ascending: false }),
        supabase
          .from("unihub_registrations")
          .select("*")
          .order("timestamp", { ascending: false }),
      ]);

      if (sData) {
        // Map DB columns (mix of camelCase + snake_case) into AppSettings
        const mappedSettings: AppSettings = {
          ...
          adminMomoName:
            sData.adminMomoName ??
            sData.admin_momo_name ??
            settings.adminMomoName,
          whatsappNumber:
            sData.whatsappNumber ??
            sData.whatsapp_number ??
            settings.whatsappNumber,
          commissionPerSeat:
            sData.commissionPerSeat ??
            sData.commission_per_seat ??
            settings.commissionPerSeat,
          shuttleCommission:
            sData.shuttleCommission ??
            sData.shuttle_commission ??
            settings.shuttleCommission,
          farePerPragia:
            sData.farePerPragia ??
            sData.fare_per_pragia ??
            settings.farePerPragia,
          farePerTaxi:
            sData.farePerTaxi ?? sData.fare_per_taxi ?? settings.farePerTaxi,
          soloMultiplier:
            sData.soloMultiplier ??
            sData.solo_multiplier ??
            settings.soloMultiplier,
          aboutMeText:
            sData.aboutMeText ?? sData.about_me_text ?? settings.aboutMeText,
          aboutMeImages:
            sData.aboutMeImages ??
            sData.about_me_images ??
            settings.aboutMeImages,
          appWallpaper:
            sData.appWallpaper ?? sData.app_wallpaper ?? settings.appWallpaper,
          appLogo: sData.appLogo ?? sData.app_logo ?? settings.appLogo,
          registrationFee:
            sData.registrationFee ??
            sData.registration_fee ??
            settings.registrationFee,
          facebookUrl:
            sData.facebookUrl ?? sData.facebook_url ?? settings.facebookUrl,
          instagramUrl:
            sData.instagramUrl ?? sData.instagram_url ?? settings.instagramUrl,
          tiktokUrl: sData.tiktokUrl ?? sData.tiktok_url ?? settings.tiktokUrl,
          adSenseClientId:
            sData.adSenseClientId ??
            sData.adsense_client_id ??
            (sData as any).adsenseclientid ??
            settings.adSenseClientId,
          adSenseSlotId:
            sData.adSenseSlotId ??
            sData.adsense_slot_id ??
            (sData as any).adsenseslotid ??
            settings.adSenseSlotId,
          adSenseLayoutKey:
            sData.adSenseLayoutKey ??
            sData.adsense_layout_key ??
            (sData as any).adsenselayoutkey ??
            settings.adSenseLayoutKey,
          adSenseStatus: (sData.adSenseStatus ??
            sData.adsense_status ??
            (sData as any).adsensestatus ??
            settings.adSenseStatus) as AppSettings["adSenseStatus"],
      ...
      if (mData) {
        const mappedMissions = mData.map((m: any) => ({
          ...m,
          entryFee: m.entryFee ?? m.entryfee ?? 0,
          driversJoined: m.driversJoined ?? m.driversjoined ?? [],
          createdAt:
            m.createdAt ??
            m.createdat ??
            m.created_at ??
  ...
  useEffect(() => {
    ...
    const channels = [
      supabase
        .channel("public:unihub_settings")
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "unihub_settings" },
          () => fetchData(),
        )
        .subscribe(),
      supabase
        .channel("public:unihub_nodes")
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "unihub_nodes" },
          () => fetchData(),
        )
        .subscribe(),
      supabase
        .channel("public:unihub_drivers")
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "unihub_drivers" },
          () => fetchData(),
        )
        .subscribe(),
      supabase
        .channel("public:unihub_missions")
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "unihub_missions" },
          () => fetchData(),
        )
        .subscribe(),
      supabase
        .channel("public:unihub_transactions")
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "unihub_transactions" },
          () => fetchData(),
        )
        .subscribe(),
      supabase
        .channel("public:unihub_topups")
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "unihub_topups" },
          () => fetchData(),
        )
        .subscribe(),
      supabase
        .channel("public:unihub_registrations")
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "unihub_registrations" },
          () => fetchData(),
        )
        .subscribe(),
    ];
  ...
  const onlineDriverCount = useMemo(
  ...
  const handleGlobalUserAuth = async (
    username: string,
    phone: string,
    pin: string,
    mode: "login" | "signup",
  ...
  const joinMission = async (missionId: string, driverId: string) => {
    ...
    await Promise.all([
      supabase
        .from("unihub_missions")
        .update({ driversjoined: newJoined })
        .eq("id", missionId),
      supabase
        .from("unihub_drivers")
        .update({
          walletBalance: latestDriver.walletBalance - mission.entryFee,
        })
        .eq("id", driverId),
    ...
    alert(
      `Successfully stationed at ${mission.location}! ₵${mission.entryFee} deducted.`,
    );
  };
  ...
  const leaveNode = async (nodeId: string, phone: string) => {
    ...
    const updatedStatus =
      newPassengers.length < node.capacityNeeded && node.status === "qualified"
        ? "forming"
        : node.status;
  ...
  const verifyRide = async (nodeId: string, code: string) => {
    ...
    const passengerMatch = node.passengers.find(
      (p) => p.verificationCode === code,
    );
  ...
  const cancelRide = async (nodeId: string) => {
    ...
    try {
      if (node.assignedDriverId) {
        ...
        if (
          isBroadcast ||
          (node.status === "forming" && node.passengers.length === 0)
        ) {
          if (node.status !== "dispatched") {
            if (isShuttle && isBroadcast) {
              const cap = node.capacityNeeded;
          ...
          alert(
            "Trip cancelled" +
              (isShuttle && isBroadcast ? " and commission refunded." : "."),
          );
          return;
        }
  ...
  const handleBroadcast = async (data: any) => {
    ...
    const activeRide = nodes.find(
      (n) => n.assignedDriverId === activeDriverId && n.status !== "completed",
    );
    if (activeRide) {
      alert("You already have an active/broadcasting trip.");
      return;
    }
    ...
    try {
      ...
      if (insError) {
        ...
        if (isShuttle) {
          console.warn(
            "Node creation failed. Initiating relative commission rollback...",
          );
  ...
  const handleStartBroadcast = async (nodeId: string) => {
    ...
    if (!isShuttle) {
      if (latestDriver.walletBalance < totalCommission) {
        alert(
          `Insufficient funds. Need ₵${totalCommission} for ${node.passengers.length} passengers.`,
        );
        return;
      }
    }
  ...
  const settleNode = async (nodeId: string) => {
    if (
      confirm(
        "Force complete this trip? (Assumes commission was prepaid or not applicable)",
      )
    ) {
      await supabase
        .from("unihub_nodes")
  ...
  const requestRegistration = async (
    reg: Omit<RegistrationRequest, "id" | "status" | "timestamp">,
  ) => {
    const existingDriver = drivers.find(
      (d) => d.contact === reg.contact || d.licensePlate === reg.licensePlate,
    );
    const existingReq = registrationRequests.find(
      (r) =>
        (r.contact === reg.contact || r.licensePlate === reg.licensePlate) &&
        r.status === "pending",
    );

    if (existingDriver) {
      alert(
        "Error: This Partner or Vehicle is already registered with NexRyde.",
      );
      return;
    }
    if (existingReq) {
      alert(
        "Application Pending: You already have an onboarding request under review.",
      );
      return;
    }
    ...
    if (error) {
      ...
      alert(
        "Application submitted! NexRyde Admin will review your details shortly.",
      );
    }
  };
  ...
  const approveTopup = async (reqId: string) => {
    ...
    await Promise.all([
      supabase
        .from("unihub_drivers")
        .update({ walletBalance: latestDriver.walletBalance + req.amount })
        .eq("id", req.driverId),
      supabase
        .from("unihub_topups")
  ...
  const registerDriver = async (
    d: Omit<Driver, "id" | "walletBalance" | "rating" | "status">,
  ) => {
    const newDriver: Driver = {
      ...d,
      id: `DRV-${Date.now()}`,
  ...
  const deleteDriver = useCallback(
    async (id: string) => {
      const hasActiveMission = nodes.some(
        (n) =>
          n.assignedDriverId === id &&
          (n.status === "qualified" || n.status === "dispatched"),
      );
      if (hasActiveMission) {
  ...
  const updateGlobalSettings = async (newSettings: AppSettings) => {
    ...
    // Map AppSettings (camelCase) into actual DB columns from Supabase schema
    const dbPayload: Record<string, unknown> = {
      ...
      // keep legacy snake_case mirrors in sync for older data
  ...
  const handleDriverAuth = async (driverId: string, password: string) => {
  console.log('🔍 Driver Auth Started:', { driverId, passwordLength: password?.length });
  
  if (!driverId || !password) {
    console.error('❌ Missing credentials:', { driverId, password });
    alert("Driver ID and Password are required.");
    return;
  }
  ...
  try {
    ...
    // Check password (prefer password field, fallback to pin for backward compatibility)
    const credentialToCheck = driver.password || driver.pin;
    if (credentialToCheck !== password) {
      console.error('❌ Password mismatch:', { 
        enteredPassword: password, 
        storedPassword: credentialToCheck ? '[REDACTED]' : null,
        passwordLength: credentialToCheck?.length 
      });
      ...
      if (shouldLock) {
        ...
        alert(`Invalid password. Attempts remaining: ${attemptsRemaining}`);
      }
      setIsSyncing(false);
      return;
    }
    ...
    try {
      await supabase.rpc('log_login_attempt', {
        ...
        p_ip_address: userIP,
        p_user_agent: userAgent,
        p_metadata: { driver_name: driver.name }
      });
        ...
        setTimeout(() => {
          const baseFare =
            newNode.vehicleType === "Taxi"
              ? settings.farePerTaxi
              : settings.farePerPragia;
          const finalFare = newNode.isSolo
            ? baseFare * settings.soloMultiplier
            : baseFare;
  ...
  return (
    ...
    >
      ...
      {showInstallBanner && (
        <div className="fixed top-0 left-0 right-0 z-[2000] bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-3 flex items-center justify-between gap-3 shadow-2xl border-b border-white/10 animate-in slide-in-from-top duration-500">
          <div className="flex items-center gap-2 min-w-0">
            {settings.appLogo ? (
              <img src={settings.appLogo} alt="" className="w-8 h-8 object-contain rounded-lg shrink-0" />
            ) : (
          ...
          <div className="flex items-center gap-2 shrink-0">
            {installPrompt && (
              <button
                ...
                className="px-3 py-1.5 bg-white text-indigo-600 rounded-lg font-black text-[9px] uppercase"
              >
                Install
              </button>
            )}
            <button
              onClick={() => {
                ...
                localStorage.setItem("nexryde_install_banner_dismissed", "1");
              }}
      ...
      {!currentUser ? (
        <HubGateway
          ...
          setShowForgotPin={setShowForgotPin}
        />
      ) : (
        <>
          {settings.hub_announcement && !dismissedAnnouncement && (
            <div className={`fixed left-0 right-0 z-[2000] bg-gradient-to-r from-amber-600 to-rose-600 px-4 py-3 flex items-start sm:items-center justify-between shadow-2xl animate-in slide-in-from-top duration-500 border-b border-white/10 ${showInstallBanner ? "top-14" : "top-0"}`}>
              <div className="flex items-start sm:items-center gap-3 flex-1">
                ...
                <p className="text-[10px] sm:text-xs font-black uppercase italic text-white tracking-tight leading-relaxed break-words">
                  {settings.hub_announcement}
                </p>
              </div>
              <button
                onClick={handleDismissAnnouncement}
                className="ml-4 w-7 h-7 rounded-full bg-black/20 flex items-center justify-center text-white text-[10px] hover:bg-white/30 transition-all shrink-0 mt-0.5 sm:mt-0"
              >
                <i className="fas fa-times"></i>
              </button>
            </div>
          ...
          {isSyncing && (
            <div
              className={`fixed ${showInstallBanner || (settings.hub_announcement && !dismissedAnnouncement) ? "top-14 sm:top-20" : "top-4"} right-4 z-[300] bg-amber-500/20 text-amber-500 px-4 py-2 rounded-full border border-amber-500/30 text-[10px] font-black uppercase flex items-center gap-2 transition-all`}
            >
          ...
          <nav className="hidden lg:flex w-72 glass border-r border-white/5 flex-col p-8 space-y-10 z-50">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-4">
                {settings.appLogo ? (
                  <img
                    src={settings.appLogo}
                    className="w-12 h-12 object-contain rounded-xl bg-white/5 p-1 border border-white/10"
                    alt="Logo"
                  />
                ) : (
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  title="NexRyde Code"
                  className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-slate-500 hover:text-amber-500 hover:bg-white/10 transition-all"
                >
                ...
                <button
                  ...
                  title="Help Center"
                  className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-slate-500 hover:text-indigo-400 hover:bg-white/10 transition-all"
                >
                ...
                <button
                  ...
                  title="Platform Info"
                  className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-slate-500 hover:text-emerald-400 hover:bg-white/10 transition-all"
                >
            ...
            <div className="flex-1 space-y-1">
              ...
              />
              <NavItem
                active={viewMode === "driver"}
                icon="fa-id-card-clip"
                label="Partner Terminal"
                onClick={() => {
              ...
              <button
                ...
                className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-slate-500 hover:bg-white/5 transition-all mt-4"
              >
            ...
            <div className="pt-6 border-t border-white/5">
              {activeDriver ? (
                <div className="bg-indigo-500/10 p-6 rounded-[2.5rem] border border-indigo-500/20 relative overflow-hidden mb-4">
                  ...
                  <button
                    ...
                    className="mt-4 w-full py-2 bg-indigo-600 rounded-xl text-[8px] font-black uppercase tracking-widest"
                  >
                    Sign Out Hub
                  </button>
                </div>
              ...
              <div className="bg-emerald-500/10 p-6 rounded-[2.5rem] border border-emerald-500/20 relative overflow-hidden">
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center">
                    <p className="text-[10px] font-black text-white uppercase opacity-60 tracking-tight">
                      Active Partners
                    </p>
          ...
          <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-20 bg-[#020617]/90 backdrop-blur-xl border-t border-white/5 z-[100] flex items-center justify-around px-4">
            <MobileNavItem
              ...
              icon="fa-user-graduate"
              label="Ride"
              onClick={() => safeSetViewMode("passenger")}
            />
            <MobileNavItem
              active={viewMode === "driver"}
              icon="fa-id-card-clip"
              label="Drive"
              onClick={() => safeSetViewMode("driver")}
            />
            {(isVaultAccess || isAdminAuthenticated) && (
              <MobileNavItem
                active={viewMode === "admin"}
          ...
          <main
            className={`flex-1 overflow-y-auto p-4 lg:p-12 pb-36 lg:pb-12 no-scrollbar z-10 relative transition-all duration-500 ${showInstallBanner ? "pt-20 sm:pt-24" : settings.hub_announcement && !dismissedAnnouncement ? "pt-24 lg:pt-28" : "pt-4 lg:pt-12"}`}
          >
            <div className="max-w-6xl mx-auto space-y-6 lg:space-y-8">
                ...
                <div className="bg-indigo-600 p-6 rounded-[2rem] shadow-2xl relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-6 animate-in slide-in-from-top-4">
                  <div className="relative z-10 flex items-center gap-6 text-center sm:text-left">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold opacity-80 mt-1 uppercase tracking-tight text-indigo-100">
                        Ready to move? Check out our quick start guide.
                      </p>
                    </div>
                  </div>
                  <div className="relative z-10 flex gap-3 w-full sm:w-auto">
                    <button
                      onClick={() => setShowHelpModal(true)}
                      className="flex-1 sm:flex-none px-6 py-3 bg-white text-indigo-600 rounded-xl font-black text-[9px] uppercase tracking-widest shadow-xl"
                    >
                      Guide
                    </button>
                    <button
                      onClick={dismissWelcome}
                      className="flex-1 sm:flex-none px-6 py-3 bg-indigo-700 text-white rounded-xl font-black text-[9px] uppercase tracking-widest"
                    >
                      Let's Go
                    </button>
                  </div>
              ...
              {viewMode === "driver" && (
                <DriverPortal
                  ...
                  onShowPassengerTracking={() => {
                    console.log('Passenger tracking clicked');
                    setShowPassengerTracking(true);
                  }}
                  onShowForgotPassword={(type: 'user' | 'driver' | 'admin') => {
                    console.log('Forgot password clicked:', type);
                ...
                (!isAdminAuthenticated ? (
                  <>
                    <AdminLogin 
                      ...
                      showAdminForgotPassword={showAdminForgotPassword}
                      setShowAdminForgotPassword={setShowAdminForgotPassword}
                    />
                    {showAdminForgotPassword && (
                      <AdminForgotPassword 
                        onClose={() => setShowAdminForgotPassword(false)}
                        onResetPassword={() => {
                  ...
                  <AdminPortal
                    ...
                    onCreateMission={async (m: any) => {
                      ...
                      await supabase.from("unihub_missions").insert([payload]);
                    }}
                    onDeleteMission={async (id: string) =>
                      await supabase
                        .from("unihub_missions")
                        .delete()
                        .eq("id", id)
            ...
            <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-end sm:items-center justify-center p-4 animate-in slide-in-from-bottom-10 fade-in">
              <div className="glass-bright w-full max-w-sm rounded-[2.5rem] p-6 space-y-6 border border-white/10 relative">
                <button
                  ...
                  className="absolute top-6 right-6 w-8 h-8 bg-white/5 rounded-full flex items-center justify-center text-slate-500 hover:text-white"
                >
                  <i className="fas fa-times"></i>
                </button>
                ...
                <div className="flex items-center gap-4">
                  {settings.appLogo ? (
                    <img
                      src={settings.appLogo}
                      alt="Logo"
                      className="w-14 h-14 object-contain rounded-2xl bg-white/5 p-1 border border-white/10 shrink-0"
                    />
                  ) : (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="p-4 bg-white/5 rounded-2xl border border-white/5 flex flex-col items-center gap-2 hover:bg-white/10"
                  >
                ...
                <AdBanner settings={settings} className="w-full" />
                ...
                <button
                  ...
                  className="w-full py-4 bg-rose-500/10 text-rose-500 border border-rose-500/20 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center gap-2"
                >
              ...
              <div className="glass-bright w-full max-sm:px-4 max-w-sm rounded-[3rem] p-10 space-y-8 animate-in zoom-in text-center border border-white/10">
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    className="flex-1 py-4 bg-white/5 rounded-[1.5rem] font-black text-[10px] uppercase text-slate-400"
                  >
                    Close
                  </button>
                  <button
                    onClick={shareHub}
                    className="flex-1 py-4 bg-amber-500 text-[#020617] rounded-[1.5rem] font-black text-[10px] uppercase shadow-xl"
                  >
                    Share Platform
                  </button>
                </div>
              ...
              <div className="glass-bright w-full max-w-4xl rounded-[3rem] p-8 lg:p-12 space-y-8 animate-in zoom-in border border-white/10 overflow-y-auto max-h-[90vh] no-scrollbar">
                <div className="flex justify-between items-center gap-4">
                  ...
                  <button
                    ...
                    className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-500 hover:text-white transition-all"
                  >
                    <i className="fas fa-times"></i>
                  </button>
                </div>
                ...
                <div className="pt-6 border-t border-white/5 text-center space-y-4">
                  <AdBanner settings={settings} className="flex justify-center" />
                  <button
                    onClick={() => setShowHelpModal(false)}
                    className="px-12 py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl"
                  >
                    Got it
                  </button>
                </div>
            ...
            <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[200] flex items-center justify-center p-4">
              <div className="glass-bright w-full max-w-2xl rounded-[3rem] p-8 lg:p-12 space-y-8 animate-in zoom-in border border-white/10 overflow-y-auto max-h-[90vh] no-scrollbar">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    {settings.appLogo ? (
                      <img src={settings.appLogo} alt="Logo" className="w-12 h-12 object-contain rounded-2xl bg-white/5 p-1 border border-white/10 shrink-0" />
                    ) : (
                  ...
                  settings.aboutMeImages.length > 0 && (
                    <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
                      {settings.aboutMeImages.map((img, i) => (
                        <div
                          key={i}
                          className="min-w-[280px] h-[180px] rounded-[2rem] overflow-hidden border border-white/10 shadow-xl shrink-0"
                        >
                          <img
                            src={img}
                            className="w-full h-full object-cover"
                            alt="NexRyde Portfolio"
                          />
                        </div>
                      ))}
                    </div>
                ...
                <div className="space-y-6">
                  <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/10 relative overflow-hidden">
                    ...
                    <p className="text-sm lg:text-base font-medium italic text-slate-300 leading-relaxed relative z-10 whitespace-pre-wrap">
                      {settings.aboutMeText}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <a
                      ...
                      target="_blank"
                      className="flex flex-col items-center gap-3 p-6 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-emerald-600/10 hover:border-emerald-500/30 transition-all group"
                    >
                    ...
                    <button
                      onClick={shareHub}
                      className="flex flex-col items-center gap-3 p-6 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-amber-600/10 hover:border-amber-500/30 transition-all group"
                    >
                    ...
                    {settings.facebookUrl && (
                      <a
                        ...
                        target="_blank"
                        className="flex flex-col items-center gap-3 p-6 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-blue-600/10 hover:border-blue-500/30 transition-all group"
                      >
                    ...
                    {settings.instagramUrl && (
                      <a
                        ...
                        target="_blank"
                        className="flex flex-col items-center gap-3 p-6 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-pink-600/10 hover:border-pink-500/30 transition-all group"
                      >
                    ...
                    {settings.tiktokUrl && (
                      <a
                        ...
                        target="_blank"
                        className="flex flex-col items-center gap-3 p-6 bg-white/5 border border-white/10 rounded-[2rem] hover:bg-gray-600/10 hover:border-gray-500/30 transition-all group"
                      >
      ...
      {showPassengerTracking && (
        <PassengerTrackingModal
          supabase={supabase}
          onClose={() => setShowPassengerTracking(false)}
        />
      )}
    </div>
  );
