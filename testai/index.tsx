...
const SNIPPETS = {
  SearchHub: `const SearchHub = ({ searchConfig, setSearchConfig }) => {
  ...
  PassengerPortal: `const PassengerPortal = ({ nodes, onJoin, onAddNode }) => {
  // 1. Filter nodes based on search
  const availableRides = nodes.filter(n => !n.isFull);
  
  // 2. Handle Realtime Updates via Props
  // 3. Render list of rides
  return (
    <div>
      {availableRides.map(node => (
        <RideCard node={node} onJoin={onJoin} />
      ))}
    </div>
  );
};`,
  VoiceOrb: `const GlobalVoiceOrb = () => {
  // 1. Connects to Gemini Multimodal Live API via WebSocket
  // 2. Streams raw PCM audio data
  // 3. Handles "Tool Calls" (Function Calling) to control app
  ...
  DriverPortal: `const DriverPortal = ({ activeDriver, missions }) => {
  // Logic for Gig Economy features:
  // - Wallet Management
  // - Geolocation (Hotspots)
  // - Broadcast Logic (Creating nodes)
  
  const acceptRide = async (rideId) => {
     // DB Transaction to deduct commission
     await supabase.from('drivers').update({ balance: newBal });
     await supabase.from('nodes').update({ driverId: activeDriver.id });
  };
};`
};
...
type PortalMode = 'passenger' | 'driver' | 'admin' | 'public' | 'learning';
...
const LessonOverlay = ({ content, onClose }: { content: LessonContent, onClose: () => void }) => {
  ...
  return (
    <div className="fixed top-24 right-4 z-[999] w-full max-w-md animate-in slide-in-from-right fade-in duration-500">
       <div className="glass-bright p-6 rounded-[2rem] border border-purple-500/30 shadow-2xl relative bg-[#0f172a]/95">
          <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 bg-white/5 rounded-full flex items-center justify-center text-slate-400 hover:text-white transition-all">
             <i className="fas fa-times"></i>
          </button>
          
          <div className="flex items-center gap-3 mb-4">
             <div className="w-10 h-10 rounded-xl bg-purple-600 flex items-center justify-center text-white shadow-lg animate-pulse">
                <i className="fas fa-chalkboard-user"></i>
             </div>
             <div>
                <h3 className="text-lg font-black italic text-white uppercase">{content.title}</h3>
                <p className="text-[10px] font-bold text-purple-400 uppercase tracking-widest">Live Lesson</p>
             </div>
          </div>

          <div className="prose prose-invert prose-sm mb-4">
             <p className="text-xs text-slate-300 font-medium leading-relaxed">{content.explanation}</p>
          </div>

          <div className="relative group">
             <div className="absolute -top-3 right-2 px-2 py-1 bg-purple-600 text-white text-[8px] font-black uppercase rounded-md shadow-md z-10">
                TypeScript
             </div>
             <pre className="bg-[#020617] p-4 rounded-xl border border-white/10 overflow-x-auto text-[10px] font-mono text-purple-200 shadow-inner">
...
const VSCodeTutor = ({ onClose }: { onClose: () => void }) => {
  ...
  const steps = [
    {
       title: "Project Setup",
       description: "Every React project starts with an entry point. We use Vite for fast builds.",
       files: {
...
<html lang="en">
  ...
  "dependencies": {
    ...
    "vite": "^5.0.0"
       ...
       explanation: "The `index.html` is the shell. React injects your app into the `<div id='root'></div>`."
    },
    {
       title: "The Entry Point",
       description: "React takes control of the DOM here.",
       files: {
...
ReactDOM.createRoot(document.getElementById('root')!).render(
  <React.StrictMode>
    <App />
       ...
       explanation: "`createRoot` creates the React application tree. `App` is our main component."
    },
    {
       title: "Building Components",
       description: "Components are the building blocks. Let's make the Ride Card.",
       files: {
          'App.tsx': `function App() {
  return (
    <div className="app-container">
       <RideCard destination="Library" price={5} />
    </div>
  )
}`,
          'RideCard.tsx': `const RideCard = ({ destination, price }) => (
  <div className="card">
     <h3>{destination}</h3>
     <p>Price: {price} GHS</p>
  </div>
       ...
       explanation: "We pass data using `props` (like `destination`). The UI is defined using JSX (HTML-in-JS)."
    },
    {
       title: "State Management",
       description: "How does the app remember things? `useState`.",
       files: {
...
const Counter = () => {
  ...
  return (
    <button onClick={() => setCount(count + 1)}>
       ...
       explanation: "`useState(0)` creates a variable `count` starting at 0. `setCount` updates it and triggers a re-render."
    },
    {
      title: "Connecting Supabase",
      description: "Real-time data from the cloud.",
      files: {
...
export const supabase = createClient(supabaseUrl, supabaseKey)`,
  ...
  fetchRides();
      ...
      explanation: "We use `useEffect` to run code when the component mounts (loads). This fetches data from our database."
    }
  ];

  const currentStepData = steps[step];
  const fileContent = currentStepData.files[activeFile as keyof typeof currentStepData.files] || "// Select a file to view content";
  const fileList = Object.keys(currentStepData.files);

  // Auto-select first file when step changes
  useEffect(() => {
     setActiveFile(fileList[0]);
  }, [step]);

  return (
     <div className="fixed inset-0 z-[1000] bg-[#1e1e1e] flex flex-col text-[#d4d4d4] font-mono animate-in zoom-in duration-300">
        {/* Title Bar */}
        <div className="h-10 bg-[#3c3c3c] flex items-center justify-between px-4 select-none">
           <div className="flex gap-2">
              <span className="w-3 h-3 rounded-full bg-[#ff5f56]"></span>
              <span className="w-3 h-3 rounded-full bg-[#ffbd2e]"></span>
              <span className="w-3 h-3 rounded-full bg-[#27c93f]"></span>
           </div>
           <span className="text-xs font-bold text-[#9ca3af]">VS Code Tutor - {currentStepData.title}</span>
           <button onClick={onClose} className="text-xs hover:text-white"><i className="fas fa-times"></i> Exit</button>
        </div>

        <div className="flex-1 flex overflow-hidden">
           {/* Sidebar */}
           <div className="w-64 bg-[#252526] flex flex-col border-r border-[#3c3c3c]">
              <div className="p-4 text-xs font-bold uppercase tracking-widest text-[#9ca3af]">Explorer</div>
              <div className="flex-1">
                 {fileList.map(f => (
                    <div 
                      key={f} 
                      onClick={() => setActiveFile(f)}
                      className={`px-4 py-1 text-sm cursor-pointer flex items-center gap-2 ${activeFile === f ? 'bg-[#37373d] text-white' : 'hover:bg-[#2a2d2e]'}`}
                    >
                       <i className={`fab ${f.endsWith('html') ? 'fa-html5 text-orange-500' : f.endsWith('css') ? 'fa-css3 text-blue-500' : f.endsWith('json') ? 'fa-node-js text-green-500' : 'fa-react text-cyan-400'}`}></i>
                       {f}
                    </div>
                 ))}
              </div>
           ...
           <div className="flex-1 bg-[#1e1e1e] flex flex-col relative">
              <div className="flex bg-[#252526]">
                 {fileList.map(f => (
                    <div key={f} onClick={() => setActiveFile(f)} className={`px-4 py-2 text-xs cursor-pointer border-r border-[#3c3c3c] ${activeFile === f ? 'bg-[#1e1e1e] text-white' : 'text-[#9ca3af]'}`}>
                       {f}
                    </div>
                 ))}
              </div>
              <div className="flex-1 p-4 overflow-auto">
                 <pre className="text-sm font-mono leading-relaxed">
                    <code dangerouslySetInnerHTML={{
                       __html: fileContent.replace(/</g, '&lt;').replace(/>/g, '&gt;')
                           .replace(/(import|export|const|function|return|var|let)/g, '<span style="color: #c586c0">$1</span>')
              ...
              {/* Terminal / Guide */}
              <div className="h-48 bg-[#1e1e1e] border-t border-[#3c3c3c] flex">
                  <div className="w-1/2 p-4 border-r border-[#3c3c3c] overflow-y-auto">
                      <div className="text-xs font-bold uppercase text-[#9ca3af] mb-2">Instructor Guide</div>
                      <h3 className="text-lg font-bold text-white mb-2">{currentStepData.title}</h3>
                      <p className="text-sm text-[#d4d4d4] leading-relaxed mb-4">{currentStepData.explanation}</p>
                      <div className="flex gap-2">
                         <button 
                           onClick={() => setStep(Math.max(0, step - 1))} 
                           disabled={step === 0}
                           className="px-4 py-2 bg-[#3c3c3c] text-white text-xs rounded hover:bg-[#4c4c4c] disabled:opacity-50"
                         >
                            Previous
                         </button>
                         <button 
                           ...
                           className="px-4 py-2 bg-[#0e639c] text-white text-xs rounded hover:bg-[#1177bb] disabled:opacity-50"
                         >
                            Next Step
                         </button>
                      </div>
                  </div>
                  <div className="w-1/2 p-4 font-mono text-xs">
                      <div className="text-[#9ca3af] mb-1">TERMINAL</div>
                      <div className="text-green-500">➜  nexryde-app git:(main) <span className="text-white">npm run dev</span></div>
                      <div className="text-white mt-2">  VITE v5.0.0  ready in 240 ms</div>
                      <div className="text-white mt-1">  ➜  Local:   <span className="text-cyan-400 underline">http://localhost:5173/</span></div>
                      <div className="text-white mt-1">  ➜  Network: use --host to expose</div>
                  </div>
              </div>
           </div>
        </div>
...
const DevPanel = ({ 
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-96 bg-[#0f172a] border-l border-white/10 p-6 z-[1000] shadow-2xl animate-in slide-in-from-right overflow-y-auto">
      <div className="flex justify-between items-center mb-8">
        <h3 className="text-xl font-black italic text-purple-400">Builder's Academy</h3>
        <button onClick={onClose} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-white"><i className="fas fa-times"></i></button>
      </div>

      <div className="mb-6">
        <h4 className="text-white font-bold text-2xl mb-2">{activeComponent.name}</h4>
        <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">Component Architecture</p>
      </div>

      <div className="space-y-4 mb-8">
        <h5 className="text-[10px] font-black uppercase text-slate-500">Key Concepts</h5>
        <div className="flex flex-wrap gap-2">
          {activeComponent.concepts.map((concept, i) => (
            <span key={i} className="px-3 py-1 bg-slate-900 border border-purple-500/30 rounded-full text-[10px] font-bold text-purple-300">
               {concept}
            </span>
          ))}
        </div>
      ...
      {activeComponent.codeSnippet && (
        <div className="mb-8">
           <div className="flex justify-between items-center mb-2">
              <h5 className="text-[10px] font-black uppercase text-slate-500">Source Code (Simplified)</h5>
              <button 
                onClick={() => onAskAi(`Explain how this code works: ${activeComponent.codeSnippet}`)}
                className="text-[9px] font-bold text-purple-400 hover:text-white flex items-center gap-1"
              >
                <i className="fas fa-magic"></i> Explain Logic
              </button>
           </div>
           <pre className="bg-[#020617] p-4 rounded-xl border border-white/10 overflow-x-auto text-[10px] font-mono text-slate-300 leading-relaxed shadow-inner">
      ...
      <div className="p-4 bg-gradient-to-br from-purple-900/40 to-indigo-900/40 rounded-2xl border border-purple-500/20">
         <div className="flex gap-4 items-start">
            <div className="w-10 h-10 rounded-full bg-purple-600 flex items-center justify-center text-white shrink-0 shadow-lg">
               <i className="fas fa-chalkboard-teacher"></i>
            </div>
            <div>
               <p className="text-xs font-bold text-purple-200 uppercase mb-1">Interactive Tutor</p>
               <p className="text-[10px] text-slate-300 leading-relaxed mb-3">"I can explain exactly how the <span className="text-white font-bold">{activeComponent.name}</span> works, theoretically and practically."</p>
               <div className="space-y-2">
                 <button 
                   onClick={() => onAskAi(`Teach me about ${activeComponent.name} and specifically ${activeComponent.concepts.join(', ')}. How do I build this?`)}
                   className="w-full py-2 bg-purple-600 text-white rounded-lg text-[10px] font-black uppercase hover:bg-purple-500 transition-colors"
                 >
                   Ask AI Tutor
                 </button>
                 <button 
                   onClick={onLaunchTutor}
                   className="w-full py-2 bg-white/10 text-white rounded-lg text-[10px] font-black uppercase hover:bg-white/20 transition-colors border border-white/10 flex items-center justify-center gap-2"
                 >
                   <i className="fas fa-laptop-code"></i> Launch VS Studio
                 </button>
               </div>
            </div>
         </div>
...
const DevModeFloat = ({ isDevMode, onToggle, onLaunchTutor }: { isDevMode: boolean, onToggle: () => void, onLaunchTutor: () => void }) => (
  <div className="fixed top-4 right-4 z-[500] flex gap-2">
    {isDevMode && (
      <button 
        onClick={onLaunchTutor}
        className="px-4 py-2 rounded-full font-black text-[10px] uppercase tracking-widest shadow-xl transition-all flex items-center gap-2 bg-[#1e1e1e] text-white border border-white/10 hover:bg-[#2e2e2e]"
      >
    ...
    <button 
      onClick={onToggle}
      className={`px-4 py-2 rounded-full font-black text-[10px] uppercase tracking-widest shadow-2xl transition-all flex items-center gap-2 ${isDevMode ? 'bg-purple-600 text-white ring-4 ring-purple-500/30' : 'bg-white/10 text-slate-500 backdrop-blur-md border border-white/10 hover:bg-white/20'}`}
    >
      <i className={`fas ${isDevMode ? 'fa-code' : 'fa-graduation-cap'}`}></i>
      {isDevMode ? 'Dev Mode On' : 'Learn Code'}
    </button>
  </div>
);
...
const GlobalVoiceOrb = ({ 
  ...
  const toggleSession = async () => {
    ...
    if (isDevMode) {
        systemInstruction = `
          You are "The Architect", a Senior Software Engineer teaching the user how this app was built.
          TECHNOLOGY STACK:
          - Frontend: React 19, TypeScript, TailwindCSS.
          - Backend: Supabase (PostgreSQL, Realtime Subscriptions).
          - AI: Google Gemini Multimodal Live API (WebSockets) and Gemini 2.5 Flash.
          
          CURRENT CONTEXT:
          ${activeComponent ? `The user is currently inspecting the ${activeComponent.name} component. 
          Concepts: ${activeComponent.concepts.join(', ')}.
          Code Snippet: "${activeComponent.codeSnippet}"` : 'The user is exploring the app overview.'}

          ROLE:
          - Explain technical concepts simply but accurately.
          - If the user asks about the component they are inspecting, use the code snippet provided to explain step-by-step.
          - Focus on: Hooks (useState, useEffect), Supabase RLS policies, and WebSocket handling for audio.
          - Do NOT act like a ride assistant. Act like a coding tutor.
          - ALWAYS USE THE 'render_lesson_card' TOOL whenever you are explaining code or a concept, so the user can see it on screen.
        `;
        tools = [
          {
            name: 'render_lesson_card',
            description: 'Display a visual lesson card on the screen with a title, markdown explanation, and code snippet.',
            parameters: {
               type: Type.OBJECT,
               properties: {
                  title: { type: Type.STRING, description: "Short title of the concept (e.g. React Hooks)" },
                  explanation: { type: Type.STRING, description: "Short, clear explanation in Markdown." },
                  codeSnippet: { type: Type.STRING, description: "The relevant code example to display." }
               },
    ...
    try {
      ...
      inputAnalyser.fftSize = 64; // Small FFT for smooth blobs
      ...
      // Connect: Stream -> Analyser -> ScriptProcessor -> Dest
        ...
        callbacks: {
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            if (audioData) {
              ...
              // Connect: Source -> OutputAnalyser -> Destination
  ...
  return (
    <>
      ...
      >
        <i className={`fas ${isActive ? 'fa-microphone-slash' : isDevMode ? 'fa-code' : 'fa-microphone'} text-white text-2xl`}></i>
      </button>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[450] flex flex-col items-center justify-center animate-in fade-in duration-300">
           ...
           <div className="mt-8 text-center px-4">
              <h3 className="text-2xl font-black italic uppercase text-white tracking-widest animate-pulse">
                {state === 'listening' ? 'Listening...' : state === 'speaking' ? (isDevMode ? 'Mentor (AI)' : 'Kofi (AI)') : 'Thinking...'}
              </h3>
              <p className="text-xs font-bold opacity-70 uppercase mt-2 tracking-[0.2em]" style={{ color: isDevMode ? '#a855f7' : mode === 'admin' ? '#f43f5e' : '#94a3b8' }}>
                {isDevMode ? 'Coding Mentor Mode' : mode === 'admin' ? 'Security Protocol Active' : mode === 'driver' ? 'Partner Hands-Free' : 'Polyglot Assistant'}
              </p>
              
              <div className="mt-8 grid grid-cols-2 gap-4 max-w-xs mx-auto text-[10px] text-slate-400 font-bold uppercase">
                 {isDevMode && (
                    <>
                       <div className="bg-purple-500/10 p-3 rounded-xl border border-purple-500/20 text-purple-400">"How does useEffect work?"</div>
                       <div className="bg-purple-500/10 p-3 rounded-xl border border-purple-500/20 text-purple-400">"Explain this component"</div>
                    </>
                 )}
...
// ... (AdminPortal remains the same, assuming it was closed in previous logic or needs to be kept if present in file. Since user provided partial files, we assume standard structure. )
...
const AiHelpDesk = ({ onClose, settings, isDevMode, context }: any) => {
...
const App: React.FC = () => {
  ...
  if (viewMode === 'learning') {
     return <VSCodeTutor onClose={() => setViewMode('passenger')} />;
  }
  ...
  return (
    ...
    >
        ...
        <div className="max-w-6xl mx-auto space-y-6 lg:space-y-8">
          ...
          {(viewMode === 'passenger' || viewMode === 'driver' || (viewMode === 'admin' && isAdminAuthenticated)) && (
            <ComponentInspector 
               isDevMode={isDevMode} 
               name="SearchHub" 
               concepts={["React Props", "State Lifting", "Filter Logic"]} 
               codeSnippet={SNIPPETS.SearchHub}
               onInspect={setInspectedComponent}
            >
          ...
          {viewMode === 'passenger' && (
            <ComponentInspector 
               ...
               concepts={["Supabase Realtime", "Array Filtering", "Complex State", "CRUD Operations"]} 
               codeSnippet={SNIPPETS.PassengerPortal}
               onInspect={setInspectedComponent}
            >
                <PassengerPortal 
          ...
          {viewMode === 'driver' && (
            <ComponentInspector 
               isDevMode={isDevMode} 
               name="DriverPortal" 
               concepts={["Auth State", "Geolocation (Simulated)", "Financial Logic", "Status Updates"]} 
               codeSnippet={SNIPPETS.DriverPortal}
               onInspect={setInspectedComponent}
            >
                <DriverPortal 
                  drivers={drivers} 
      ...
      <DevPanel 
         ...
         onAskAi={(question) => {
            setShowAiHelp(true);
            // We can pre-fill or rely on context prop
         }}
         onLaunchTutor={() => setViewMode('learning')}
      />
      ...
      {showMenuModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[300] flex items-end sm:items-center justify-center p-4 animate-in slide-in-from-bottom-10 fade-in">
           <div className="glass-bright w-full max-w-sm rounded-[2.5rem] p-6 space-y-6 border border-white/10 relative">
              ...
              <button 
                 onClick={() => { setIsDevMode(!isDevMode); setShowMenuModal(false); }} 
                 className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all border ${isDevMode ? 'bg-purple-600/20 border-purple-500 text-purple-400' : 'bg-white/5 border-transparent text-slate-500'}`}
              >
                 <i className="fas fa-code text-lg w-6"></i>
                 <div className="text-left">
                    <span className="block text-sm font-bold">Dev Mode</span>
                    <span className="text-[9px] uppercase tracking-widest">{isDevMode ? 'Learning Active' : 'Enable Learning'}</span>
                 </div>
              </button>
