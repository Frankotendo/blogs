...
export const GlobalVoiceOrb = ({ 
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (state === 'speaking' && analyserRef.current) {
          ...
          let sum = 0;
          // Focus on the lower frequencies where voice usually is
          const limit = Math.floor(dataArray.length / 2); 
          for(let i = 0; i < limit; i++) {
              sum += dataArray[i];
          }
          const avg = sum / limit;
          // Amplify the effect for visual clarity
          pulse = (avg / 255) * 30; 
          // Add minimal jitter for liveness
          pulse += Math.random() * 2;
      } else {
          // Standard breathing animation for other states
          const pulseSpeed = state === 'listening' ? 6 : (state === 'processing' ? 12 : 3);
          pulse = Math.sin(time * pulseSpeed) * 5;
      }
  ...
  const toggleSession = async () => {
    ...
    const ghanaianPersona = `You are "Kofi", the NexRyde Assistant. Speak English, Twi, Ga, or Pidgin. Be concise. You are an expert on Ghana geography, landmarks (KNUST, Accra, Kumasi), and global locations.`;
        ...
        callbacks: {
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            if (audioData) {
              ...
              // Connect source -> analyser -> destination for visualization
              if (analyserRef.current) {
                  source.connect(analyserRef.current);
                  analyserRef.current.connect(outputAudioContext.destination);
              } else {
            ...
            if (msg.toolCall) {
              ...
              for (const fc of msg.toolCall.functionCalls) {
                 ...
                 if (fc.name === 'navigate_ui' && currentActions.onNavigate) {
                    ...
                    setOrbMessage(`Navigating to ${args.target}...`);
                    currentActions.onNavigate(args.target, args.sub_section, args.modal);
                    result = { result: `Success. User is now viewing ${args.target} ${args.sub_section ? '- ' + args.sub_section : ''}.` };
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[450] flex flex-col items-center justify-center animate-in fade-in duration-300">
           ...
           <div className="mt-8 text-center px-4">
              <h3 className="text-2xl font-black italic uppercase text-white tracking-widest animate-pulse">
                {state === 'listening' ? 'Tie me...' : state === 'speaking' ? 'Kofi (AI)' : state === 'processing' ? 'Working...' : 'Thinking...'}
              </h3>
  ...
  useEffect(() => {
    if (!chatRef.current) {
        chatRef.current = ai.chats.create({
            ...
            config: {
                systemInstruction: `You are the help desk for NexRyde. Be concise. Context: ${settings.aboutMeText}. Support contacts: ${settings.whatsappNumber}.`,
            }
        });
        setMessages([{ role: 'model', text: "Hello! How can I help you with NexRyde today?" }]);
    }
  }, [settings]);
        ...
        <div className="glass w-full max-w-md h-[600px] max-h-[90vh] rounded-[2.5rem] border border-white/10 flex flex-col overflow-hidden animate-in zoom-in">
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-sm font-black uppercase text-white">NexRyde Support</h3>
