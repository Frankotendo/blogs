...
export const App: React.FC = () => {
  ...
  // ADDED: Global session integrity check
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const publicChannels = [
      ...
      supabase.channel('public:unihub_topups').on('postgres_changes', { event: '*', schema: 'public', table: 'unihub_topups' }, () => { if(isAdminAuthenticated || currentUser) fetchPrivateData(); }).subscribe(),
    ];
  ...
  // HUB REVENUE: Sum of commissions and registration fees
  const hubRevenue = useMemo(() => 
    transactions
      .filter(tx => tx.type === 'commission' || tx.type === 'registration')
  ...
  const handleGlobalUserAuth = async (username: string, phone: string, hashedPin: string, mode: 'login' | 'signup', plainPin?: string) => {
    ...
    try {
      ...
      if (mode === 'login') {
        if (!data) {
          alert("Profile not found! Please check your number or Create a Profile.");
          setIsSyncing(false);
          return;
        }
        ...
        // Handle legacy vs new hashing
        if (user.pin && user.pin.length < 60 && plainPin) {
             if (user.pin === plainPin) {
                 isValid = true;
                 await supabase.from('unihub_users').update({ pin: hashedPin }).eq('id', user.id);
             }
        } else if (user.pin === hashedPin) {
             isValid = true;
        }
  ...
  const handleManualCredit = async (identifier: string, amount: number) => {
      ...
      const { data: driverData } = await supabase.from('unihub_drivers').select('*').eq('contact', identifier).maybeSingle();
      if (driverData) {
          const newBal = (driverData.walletBalance || 0) + amount;
          await supabase.from('unihub_drivers').update({ walletBalance: newBal }).eq('id', driverData.id);
          alert(`Credited Driver ${driverData.name} with ₵${amount}. New Balance: ₵${newBal}`);
          return;
      }

      alert("User or Driver not found with that phone number.");
  };
  ...
  const leaveNode = async (nodeId: string, phone: string) => {
    ...
    // Check if this action should incur a penalty
    if (node.status === 'dispatched' && currentUser) {
        if (confirm("Leaving an active dispatched trip will incur a ₵1.00 Time-Waste penalty. Proceed?")) {
            const bal = currentUser.walletBalance ?? (currentUser as any).wallet_balance ?? 0;
            const newBal = Number(bal) - 1.00;
            
            await supabase.from('unihub_users').update({ walletBalance: newBal }).eq('id', currentUser.id);
            await supabase.from('unihub_transactions').insert([{
                id: `TX-PENALTY-P-${Date.now()}`,
                userId: currentUser.id,
                amount: 1.00,
                type: 'commission',
                reference: `CANCEL-${nodeId}`,
                timestamp: new Date().toLocaleString()
            }]);
            alert("₵1.00 Penalty applied for late cancellation.");
        } else {
            return;
        }
    }
    ...
    if (newPassengers.length < node.capacityNeeded && node.status === 'qualified') {
        updatedStatus = 'forming';
    } else if (newPassengers.length === 0 && node.status === 'dispatched') {
        // If the last passenger leaves a dispatched ride, reset it to empty/forming
        updatedStatus = 'forming';
    }
    ...
    await supabase.from('unihub_nodes').update({ 
        passengers: newPassengers, 
        status: updatedStatus,
        assignedDriverId: newPassengers.length === 0 ? null : node.assignedDriverId
    }).eq('id', nodeId);
      ...
      try {
        ...
        for (const p of node.passengers) {
             ...
             if (uData) {
                 ...
                 await supabase.from('unihub_users').update(updates).eq('id', uData.id).then(r => {
                     if (r.error) return supabase.from('unihub_users').update({ wallet_balance: newBalance }).eq('id', uData.id);
                     return r;
                 });
        ...
        // Credit driver and LOG COMMISSION
        await Promise.all([
          supabase.from('unihub_drivers').update({ walletBalance: (driver.walletBalance ?? 0) + driverEarnings }).eq('id', driverId),
          supabase.from('unihub_transactions').insert([{
            id: `TX-COMM-${Date.now()}`,
            driverId: driverId,
        ...
        removeRideFromMyList(nodeId);
        alert(passengerMatch ? `Ride Verified for ${passengerMatch.name}! Earnings credited.` : `Ride verified and completed! Earnings credited.`);
      } catch (err: any) {
  ...
  const cancelRide = async (nodeId: string, role: 'passenger' | 'driver' | 'admin' = 'admin') => {
    ...
    try {
      // 1. Logic for Driver Cancelling an active trip
      if (role === 'driver' && node.status === 'dispatched' && activeDriverId) {
          ...
          alert("Trip Reset. ₵2.00 penalty applied to your earnings wallet.");
          return;
      }

      // 2. Logic for Passenger Cancelling (already handled in leaveNode for individuals, this handles the whole node)
      if (role === 'passenger' && node.status === 'dispatched' && currentUser) {
          if (!confirm("Cancelling a dispatched ride will incur a ₵1.00 Time-Waste penalty. Proceed?")) return;
          ...
          await Promise.all([
              ...
              supabase.from('unihub_transactions').insert([{
                  id: `TX-PENALTY-P-FULL-${Date.now()}`,
                  userId: currentUser.id,
                  amount: 1.00,
                  type: 'commission',
                  reference: `CANCEL-FULL-${nodeId}`,
                  timestamp: new Date().toLocaleString()
              }]),
              supabase.from('unihub_nodes').delete().eq('id', nodeId)
          ]);
          removeRideFromMyList(nodeId);
          alert("Ride Request Deleted. ₵1.00 penalty applied.");
          return;
      }

      // 3. Fallback for non-dispatched or Admin cancellations
  ...
  // Passenger top-up now strictly requires admin approval
  const handlePassengerTopup = async (amount: number, ref: string, proofImage: string) => {
      ...
      const req: TopupRequest = {
          ...
          amount: amount,
          momoReference: ref,
          status: 'pending', // CHANGE: Force pending state
          timestamp: new Date().toLocaleString(),
      ...
      try {
          ...
          alert(`Receipt for ₵${amount} submitted! Funds will reflect in your wallet once verified by an Admin.`);
      } catch (err: any) {
          console.error("Topup submission failed", err);
          alert("System Error: Could not submit topup. Please contact admin.");
      }
  };
  ...
  const approveTopup = async (reqId: string) => {
    ...
    try {
        if (req.driverId && !req.userId) { // Withdrawal logic (no user ID means driver-only withdrawal)
        ...
        } else if (req.userId) { // Topup logic (user ID present)
            ...
            if (user) {
                ...
                const updates: any = { walletBalance: newBal };
                await supabase.from('unihub_users').update(updates).eq('id', user.id).then(r => {
                    if (r.error) return supabase.from('unihub_users').update({ wallet_balance: newBal }).eq('id', user.id);
                    return r;
                });
        ...
        alert("Request Approved and Wallet Updated.");
    } catch (e: any) {
            ...
            <div className="max-w-6xl mx-auto space-y-6 lg:space-y-8">
              ...
              {viewMode === 'passenger' && (
                <PassengerPortal 
                  ...
                  onCancel={(id: string) => cancelRide(id, 'passenger')} 
                  drivers={drivers} 
              ...
              {viewMode === 'driver' && (
                <DriverPortal 
                  ...
                  onVerify={verifyRide}
                  onCancel={(id: string) => cancelRide(id, 'driver')}
              ...
              {viewMode === 'admin' && (
                !isAdminAuthenticated ? (
                  ...
                  <AdminPortal 
                    ...
                    onCancelRide={(id: string) => cancelRide(id, 'admin')}
