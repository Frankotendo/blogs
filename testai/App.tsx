...
export const App: React.FC = () => {
  ...
  // Optimized Fetching: Split into Public vs Auth Data
  ...
  const fetchPrivateData = async () => {
      // Fetch data relevant for authorized users (Admin or Logged In)
      // Only admins need full transactions. 
      // Regular users need drivers to see partners.
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      setIsAdminAuthenticated(!!session);
      fetchData(); // Refetch on auth change
    });

    fetchData();

    // Optimizing subscriptions - Only subscribe to global events always
    ...
    if (isAdminAuthenticated) {
        privateChannels = [
            supabase.channel('public:unihub_transactions').on('postgres_changes', { event: '*', schema: 'public', table: 'unihub_transactions' }, () => fetchPrivateData()).subscribe(),
            supabase.channel('public:unihub_topups').on('postgres_changes', { event: '*', schema: 'public', table: 'unihub_topups' }, () => fetchPrivateData()).subscribe(),
            supabase.channel('public:unihub_registrations').on('postgres_changes', { event: '*', schema: 'public', table: 'unihub_registrations' }, () => fetchPrivateData()).subscribe()
        ];
    }

    return () => {
      publicChannels.forEach(ch => supabase.removeChannel(ch));
      privateChannels.forEach(ch => supabase.removeChannel(ch));
      subscription.unsubscribe();
    };
  }, [isAdminAuthenticated, currentUser?.id]); // Re-subscribe when auth changes
    ...
    try {
      ...
      if (mode === 'login') {
        ...
        if (user.pin && user.pin.length < 60 && plainPin) {
             // Likely plaintext in DB. Check against plainPin input.
  ...
  const handleManualCredit = async (identifier: string, amount: number) => {
      // Identifier can be phone or name match
      // First check users
