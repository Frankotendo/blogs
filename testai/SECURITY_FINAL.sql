-- Security Functions - Final Version with Policy Handling
-- This script handles existing policies and functions safely

-- First, drop existing policies to avoid conflicts
DROP POLICY IF EXISTS "Users can manage own security answers" ON user_security_answers;
DROP POLICY IF EXISTS "Drivers can manage own security answers" ON driver_security_answers;
DROP POLICY IF EXISTS "Users can view own security logs" ON unihub_security_logs;
DROP POLICY IF EXISTS "Users can insert own security logs" ON unihub_security_logs;
DROP POLICY IF EXISTS "Everyone can view active security questions" ON security_questions;
...
-- Create policies for security logs (only if they don't exist)
CREATE POLICY IF NOT EXISTS "Users can view own security logs" ON unihub_security_logs
  FOR SELECT USING (metadata->>'identifier' = auth.uid()::text);

CREATE POLICY IF NOT EXISTS "Users can insert own security logs" ON unihub_security_logs
  FOR INSERT WITH CHECK (metadata->>'identifier' = auth.uid()::text);

-- Create policies for security questions (only if they don't exist)
CREATE POLICY IF NOT EXISTS "Everyone can view active security questions" ON security_questions
  FOR SELECT USING (is_active = true);

-- Create policies for user security answers (only if they don't exist)
CREATE POLICY IF NOT EXISTS "Users can manage own security answers" ON user_security_answers
  FOR ALL USING (user_id = auth.uid()::text);

-- Create policies for driver security answers (only if they don't exist)
CREATE POLICY IF NOT EXISTS "Drivers can manage own security answers" ON driver_security_answers
