...
import { MapPin, Navigation, Clock, Users, Car, ExternalLink, Copy, Check } from 'lucide-react';
...
interface EnhancedRouteDirectionsProps {
  driverId: string;
  activeRideId?: string;
  className?: string;
}

const EnhancedRouteDirections: React.FC<EnhancedRouteDirectionsProps> = ({ 
  driverId, 
  activeRideId,
  className = '' 
}) => {
  ...
  const fetchActiveRide = async () => {
    ...
    try {
      ...
      const mockRide: DriverRide = {
        id: activeRideId,
        origin: 'Accra Mall',
        destination: 'Airport',
        passengers: [
          { id: '1', name: 'John Doe', phone: '+233201234567' },
          { id: '2', name: 'Jane Smith', phone: '+233207654321' }
        ],
        status: 'active'
      };
    ...
    <div className={`bg-slate-800 rounded-2xl border border-slate-700 shadow-xl ${className}`}>
      ...
      <div className="bg-slate-900 border-b border-slate-700 px-6 py-4 rounded-t-2xl">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-white font-bold text-lg">Enhanced Pickup Directions</h3>
        </div>
      </div>

      {/* Active Ride Info */}
      {activeRide && (
        <div className="bg-blue-600/10 border border-blue-500/20 px-6 py-4">
          <div className="flex items-center gap-2 mb-3">
            <Car className="w-5 h-5 text-blue-400" />
            <div>
              <h4 className="text-white font-bold">Current Active Ride</h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="text-blue-200 text-sm">
              <span className="font-bold text-white">Passengers:</span> {activeRide.passengers.length}
            </div>
            <div className="text-blue-200 text-sm">
              <span className="font-bold text-white">Status:</span> {activeRide.status}
            </div>
          </div>
        </div>
      )}
...
export default EnhancedRouteDirections;
