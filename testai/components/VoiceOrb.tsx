...
import { Type, Modality, FunctionDeclaration, LiveServerMessage } from "@google/genai";
...
export const GlobalVoiceOrb = ({ 
  ...
  const toggleSession = async () => {
    ...
    // Internal tool to trigger greeting
    const greetTool: FunctionDeclaration = {
      name: 'greet_user',
      description: 'Call this when you start the session to greet the user.',
      parameters: { type: Type.OBJECT, properties: {} }
    };

    let tools: FunctionDeclaration[] = [greetTool];
    let systemInstruction = "";

    const ghanaianPersona = `You are Kofi, a helpful Ghanaian AI assistant for the NexRyde campus logistics app. Speak naturally in a mix of English, Twi, and Pidgin. Be concise but warm. Use phrases like "Safe journey", "Akwaaba", or "Charley" occasionally.`;

    if (mode === 'driver') {
      systemInstruction = `${ghanaianPersona} You are talking to Partner ${user?.name || 'Driver'}. Help them manage status and earnings. YOU MUST GREET THEM IMMEDIATELY when the session starts. Tools: update_status, check_wallet, greet_user.`;
      tools.push({
          name: 'update_status',
      ...
      systemInstruction = `${ghanaianPersona} You are the Terminal Overseer. Talk to Admin ${user?.name || 'Manager'}. Report stats. YOU MUST GREET THEM IMMEDIATELY. Tools: get_revenue_report, system_health_check, greet_user.`;
      tools.push({ name: 'get_revenue_report', description: 'Show total money.' }, { name: 'system_health_check', description: 'Show app counts.' });
    } else if (mode === 'public') {
       systemInstruction = `${ghanaianPersona} You are the Gateway Guard. Help the user log in or sign up. YOU MUST GREET THEM IMMEDIATELY. Extract credentials: name, phone, pin via fill_auth_details. Tools: fill_auth_details, greet_user.`;
       tools.push({
      ...
      systemInstruction = `${ghanaianPersona} You are talking to Student ${user?.username || 'Rider'}. Help them book rides. YOU MUST GREET THEM IMMEDIATELY. Tools: fill_ride_form, confirm_ride, greet_user.`;
      tools.push({ 
          name: 'fill_ride_form', 
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            ...
            // Trigger Kofi's voice immediately
            sessionPromise.then(session => {
              // Send a tiny audio buffer to wake up the stream context if needed
              const silence = new Float32Array(1024).fill(0);
              session.sendRealtimeInput({ media: createBlob(silence) });
              
              // Send a valid tool response for the greet_user tool we declared
              session.sendToolResponse({
                functionResponses: {
                  id: "init_" + Date.now(),
                  name: "greet_user",
                  response: { result: "The session has successfully started. Please introduce yourself and ask how you can help." }
                }
              });
            });
          ...
          onmessage: async (msg: LiveServerMessage) => {
            ...
            if (msg.toolCall) {
              ...
              for (const fc of msg.toolCall.functionCalls) {
                 ...
                 } else if (fc.name === 'greet_user') {
                     result = { result: "Kofi is now online and greeting the user." };
                 }
      ...
      console.error("AI Bridge Failed to Initialize:", e);
      setState('error');
      vibrate([100, 50, 100]);
      setTimeout(() => setIsActive(false), 3000);
    }
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-[450] flex flex-col items-center justify-center animate-in fade-in duration-300">
           ...
           <div className="mt-8 text-center px-4">
              <h3 className="text-2xl font-black italic uppercase text-white tracking-widest animate-pulse">
                {state === 'connecting' ? 'SYNCING...' : state === 'listening' ? 'Kofi Listening' : state === 'speaking' ? 'Kofi Speaking' : state === 'error' ? 'Connection Error' : '...'}
              </h3>
              <p className="text-xs font-bold text-indigo-400 uppercase mt-2 tracking-[0.2em]">
                {state === 'error' ? 'Please check your connection and try again.' : (state === 'speaking' ? 'Kofi is responding...' : 'NexRyde Real-time Gateway')}
              </p>
           </div>
