...
export const PassengerPortal = ({ 
  ...
  useEffect(() => {
    let base = nodeData.vehicleType === 'Taxi' ? (settings.farePerTaxi || 0) : (settings.farePerPragia || 0);
    if (nodeData.isSolo) base *= (settings.soloMultiplier || 1);
    setFareEstimate(base);
    
    setCustomOffer(null);
    setOfferInput(base.toFixed(2));
  }, [nodeData.vehicleType, nodeData.isSolo, settings]);
  ...
  const toggleSolo = () => {
    if (nodeData.isSolo) {
      setNewNode({...nodeData, isSolo: false});
    } else {
      if (isSoloUnlocked) {
        setNewNode({...nodeData, isSolo: true});
      } else {
        setShowSoloAd(true);
      }
    }
  ...
  const handleSubmit = () => {
    if (!nodeData.origin || !nodeData.destination) return alert("Please fill all fields");
    ...
    const node: RideNode = {
      ...
      isSolo: nodeData.isSolo,
      capacityNeeded: nodeData.isSolo ? 1 : (nodeData.vehicleType === 'Taxi' ? 4 : 3),
      passengers: [{ id: currentUser.id, name: currentUser.username, phone: currentUser.phone }],
      status: nodeData.isSolo ? 'qualified' : 'forming',
      ...
      <div className="glass p-8 rounded-[2.5rem] border border-white/10 animate-in zoom-in max-w-lg mx-auto relative">
         ...
         <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2 p-1 bg-white/5 rounded-2xl">
               <button onClick={() => setNewNode({...nodeData, isSolo: false})} className={`py-3 rounded-xl font-black text-[10px] uppercase transition-all ${!nodeData.isSolo ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}>Pool (Cheaper)</button>
               <button onClick={toggleSolo} className={`py-3 rounded-xl font-black text-[10px] uppercase transition-all flex items-center justify-center gap-2 ${nodeData.isSolo ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}>
                 Solo (Express)
                 {!isSoloUnlocked && !nodeData.isSolo && <i className="fas fa-lock text-[8px] opacity-70"></i>}
               </button>
            </div>
            
            <div className="relative">
              <input value={nodeData.origin} onChange={e => setNewNode({...nodeData, origin: e.target.value})} placeholder="Pickup Location" className="w-full bg-white/5 border border-white/10 rounded-xl pl-4 pr-10 py-3 text-white font-bold outline-none text-sm focus:border-indigo-500" />
            ...
            <div className="relative">
              <input value={nodeData.destination} onChange={e => setNewNode({...nodeData, destination: e.target.value})} placeholder="Dropoff Location" className="w-full bg-white/5 border border-white/10 rounded-xl pl-4 pr-10 py-3 text-white font-bold outline-none text-sm focus:border-indigo-500" />
              <button onClick={onTriggerVoice} className="absolute right-2 top-2 w-8 h-8 flex items-center justify-center text-indigo-400 hover:text-white"><i className="fas fa-microphone"></i></button>
            </div>
            
            <div className="flex gap-2 overflow-x-auto no-scrollbar py-2">
               {['Pragia', 'Taxi', 'Shuttle'].map(v => (
                 <button key={v} onClick={() => setNewNode({...nodeData, vehicleType: v as any})} className={`px-4 py-2 rounded-lg border border-white/10 font-black text-[10px] uppercase ${nodeData.vehicleType === v ? 'bg-amber-500 text-[#020617]' : 'bg-white/5 text-slate-400'}`}>
                    {v}
                 </button>
               ))}
            </div>
                       ...
                       <div className="space-y-4 animate-in slide-in-from-bottom">
                           {scanResult.valid ? (
                               <div className="bg-emerald-500/10 p-6 rounded-2xl border border-emerald-500/30 text-center">
                                   ...
                                   <h4 className="text-lg font-black text-white uppercase">Receipt Logged</h4>
                                   <p className="text-3xl font-black text-emerald-400 my-2">₵{scanResult.amount}</p>
                                   <p className="text-[9px] text-slate-400 font-mono mb-4 italic">Waiting for Admin to release funds...</p>
                                   <button onClick={finalizeTopup} className="w-full mt-4 py-3 bg-emerald-500 text-[#020617] rounded-xl font-black text-[10px] uppercase shadow-xl">Understood</button>
                               </div>
                           ) : (
                               <div className="bg-rose-500/10 p-6 rounded-2xl border border-rose-500/30 text-center">
                                   ...
                                   <h4 className="text-lg font-black text-white uppercase">Verification Failed</h4>
