...
import { formatCoordinatesForDB } from './coordinate_utils';
    ...
    if ('geolocation' in navigator) {
      const watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          setPassengerLocation({ lat: validCoords.latitude, lng: validCoords.longitude });
          
          // Update existing marker instead of creating new one
          if (passengerMarkerRef.current) {
            passengerMarkerRef.current.setLatLng([validCoords.latitude, validCoords.longitude]);
          } else {
            ...
            const marker = window.L.marker([validCoords.latitude, validCoords.longitude], { icon: passengerIcon })
              .addTo(leafletMap)
              .bindPopup('Your Location');
            
            passengerMarkerRef.current = marker;
            leafletMap.setView([validCoords.latitude, validCoords.longitude], 15);
          }
        },
        (error) => {
